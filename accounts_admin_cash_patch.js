(function(){
  ...
  function aliasName(name=''){
    ...
    if (!s) return '';
    if (s.includes('boda') || s.includes('Boda') || s.toLowerCase()==='abdelrahman') return 'بودا';
  ...
  function adminCashRowActionLabel(row={}){
    if (String(row?.source_type || '').trim() === 'cash_adjustment') return Number(row?.delta) >= 0 ? 'تعديل سيولة (زيادة)' : 'تعديل سيولة (نقصان)';
    return adminCashActionLabel(row?.entry_kind);
  }
  ...
  function adminCashSourceLabel(row={}){
    ...
    if(source==='cash_adjustment') return cleanNote || 'تعديل السيولة الحالية';
    if(source==='fixed_asset') return cleanNote || 'شراء أصل ثابت';
...
function expenseStepTypeFromField(field=''){
  return ({ cost_zinc:'plate', cost_print:'print', cost_lamination:'lamination', cost_breaking:'breaking', cost_make:'make', cost_hand_fix:'handle' })[String(field || '').trim()] || '';
}
...
async function syncExecutionPartnerFromOrder(){
  ...
  function expenseExecStepLabel(field=''){
    if (field === 'cost_zinc') return 'الزنكات';
    if (field === 'cost_print') return 'الطباعة';
    if (field === 'cost_lamination') return 'السلوفان';
    if (field === 'cost_breaking') return 'التكسير';
    if (field === 'cost_make') return 'التصنيع';
    if (field === 'cost_hand_fix') return 'تركيب اليد';
    return '';
  }
  function expenseExecPartnerType(field=''){
    if (field === 'cost_zinc') return 'زنكات';
    if (field === 'cost_print') return 'مطبعة';
    if (field === 'cost_lamination') return 'سلوفان';
    if (field === 'cost_breaking') return 'تكسير';
  ...
  async function refreshAdminCashDetails(){
    ...
    /* الخزنة نفسها بند مستقل جوه summary، فجمعها مع عهد المستخدمين كان بيخلي
       "المتبقي بالخزنة" يطرح الخزنة من نفسها ويطلع رقم وهمي. */
    const isCashboxRow = (row) => String(actorUsername(row) || '').trim() === '__cashbox__';
    const cashboxRow = summary.find(isCashboxRow) || null;
    const drawerCash = cashboxRow ? toNum(cashboxRow.current_balance) : 0;
    const totalAdminBalance = summary.filter(row => !isCashboxRow(row)).reduce((sum,row)=>sum + toNum(row.current_balance), 0);
    ...
    const cashGap = Math.round((globalCash - (drawerCash + totalAdminBalance)) * 100) / 100;
    const cardsHtml = summary.length ? summary.map(row=>{
      const last = row.last_entry;
      const lastText = last ? `${escHtml(last.entry_date||'-')} | ${escHtml(adminCashRowActionLabel(last))} | ${fmtMoney(last.delta||0)}` : '-';
    ...
    box.innerHTML = `
      <div class="admin-cash-pills">
        ...
        <span class="admin-cash-pill">إجمالي عهد الشغل (المستخدمين)<b>${fmtMoney(totalAdminBalance)}</b></span>
        <span class="admin-cash-pill">المتبقي بالخزنة/الشركة<b>${fmtMoney(drawerCash)}</b></span>
        ${Math.abs(cashGap) >= 0.01 ? `<span class="admin-cash-pill" style="border-color:rgba(248,113,113,.45);color:#fca5a5">فرق غير مفسر بين السيولة وسجل الخزنة<b>${fmtMoney(cashGap)}</b></span>` : ''}
      </div>
