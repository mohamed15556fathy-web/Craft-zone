...
(function(){
  ...
  function renderPartnerFundSummary(){
    ...
    if (!summary) return;
    const allBalance = PARTNERS.reduce((sum,name)=>sum + Number(partnerFundStats(name).balance||0),0);
    summary.innerHTML = `<span class="partner-pill">إجمالي عهدة الشغل<b>${money(allBalance)}</b></span>` + PARTNERS.map(name=>`<span class="partner-pill">${esc(name)} معه<b>${money(partnerFundStats(name).balance)}</b></span>`).join('');
  }
...
function renderPartnerFundTable(){
  ...
  window.savePartnerFundEntry = async function(){
    ...
    if (!PARTNERS.includes(body.partner_name)) return alert('اختار شريك نشط');
