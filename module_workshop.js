...
function register(ctx) {
  ...
  // تعديل سطر يومية واحد بالـ id (مش بالـ worker+date زي الحفظ الجماعي)،
  // عشان التعديل من سجل الشغل يبقى في مكانه على نفس السطر.
  app.post('/update-workshop-attendance/:id', authRequired, canManage, async (req, res) => {
    try {
      const id = num(req.params.id);
      const row = await getAsync(`SELECT * FROM workshop_attendance WHERE id=?`, [id]);
      if (!row) return res.status(404).json({ error: 'السطر مش موجود' });
      const worker = await getAsync(`SELECT * FROM workshop_workers WHERE id=?`, [num(row.worker_id)]);

      const b = req.body || {};
      const given = v => !(v === undefined || v === null || clean(v) === '');
      const payType = given(b.pay_type) ? payTypeOf(b.pay_type) : payTypeOf(row.pay_type);
      const workDate = given(b.work_date) ? clean(b.work_date) : clean(row.work_date);

      // لو النظام اتغيّر، بناخد القيمة الافتراضية من ملف الصنايعي بدل ما نسيبها صفر.
      const fraction = payType === 'hourly' ? 0
        : Math.max(0, given(b.day_fraction) ? num(b.day_fraction) : (num(row.day_fraction) || 1));
      const hours = payType === 'hourly'
        ? Math.max(0, given(b.hours) ? num(b.hours) : (num(row.hours) || num(worker?.default_hours, 8)))
        : 0;
      const wage = payType === 'hourly' ? 0
        : roundMoney(given(b.daily_wage) ? num(b.daily_wage) : (num(row.daily_wage) || num(worker?.daily_wage)));
      const rate = payType === 'hourly'
        ? roundMoney(given(b.hourly_rate) ? num(b.hourly_rate) : (num(row.hourly_rate) || num(worker?.hourly_rate)))
        : 0;
      const extra = roundMoney(given(b.extra_amount) ? num(b.extra_amount) : num(row.extra_amount));
      const deduction = roundMoney(given(b.deduction_amount) ? num(b.deduction_amount) : num(row.deduction_amount));
      const note = b.note === undefined ? clean(row.note) : clean(b.note);
      const amount = computeAttendanceAmount({ payType, fraction, wage, hours, rate, extra, deduction });

      // في فهرس فريد على (worker_id, work_date)، فلازم نمنع التصادم برسالة مفهومة.
      if (workDate !== clean(row.work_date)) {
        const clash = await getAsync(`SELECT id FROM workshop_attendance WHERE worker_id=? AND work_date=? AND id<>?`, [num(row.worker_id), workDate, id]);
        if (clash) return res.status(409).json({ error: `${row.worker_name} عنده يومية مسجلة بالفعل في ${workDate}` });
      }

      await runAsync(`UPDATE workshop_attendance SET work_date=?,pay_type=?,day_fraction=?,daily_wage=?,hours=?,hourly_rate=?,amount=?,extra_amount=?,deduction_amount=?,note=? WHERE id=?`,
        [workDate, payType, fraction, wage, hours, rate, amount, extra, deduction, note, id]);

      const changes = [];
      if (clean(row.work_date) !== workDate) changes.push(`التاريخ ${row.work_date} ← ${workDate}`);
      if (payTypeOf(row.pay_type) !== payType) changes.push(`النظام ${payTypeOf(row.pay_type) === 'hourly' ? 'بالساعة' : 'باليومية'} ← ${payType === 'hourly' ? 'بالساعة' : 'باليومية'}`);
      if (roundMoney(num(row.amount)) !== amount) changes.push(`المستحق ${roundMoney(num(row.amount))} ← ${amount}`);
      await recordAudit({ req, action: 'update-workshop-attendance', entity_type: 'workshop_attendance', entity_id: id,
        details: `تعديل يومية ${row.worker_name}${changes.length ? ' | ' + changes.join(' | ') : ''}` });

      const updated = await getAsync(`SELECT * FROM workshop_attendance WHERE id=?`, [id]);
      res.json({ success: true, row: updated, amount });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
