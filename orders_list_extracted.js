...
let currentReshipOrderId=0;
let currentReshipGroupCode='';
...
let selectedStatusFilters=new Set(ALL_ORDER_STATUSES);
...
function normalizePaperType(v){v=String(v||'').trim();return(v==='كوشيه'||v==='برستول كوشيه'||v==='ورق طبع'||v==='ورق زبدة')?v:'كرافت';}
...
function isFixedWhitePaperType(v){const t=normalizePaperType(v);return isCoatedPaperType(t)||t==='ورق طبع'||t==='ورق زبدة';}
...
function rebuildEditPrintTypeOptions(){
  ...
  let list;
  // ورق الزبدة بيدخل طباعة أوفست بس.
  if(isButterPaperType(type)) list=['أوفست'];
  else if(useReady) list=isCoatedPaperType(type)?['سادة']:['سادة','سلك سكرين'];
...
function currentOrderShippingCost(o){
...
function previousOrderShippingCost(o){ return Math.max(0, Number(o?.previous_shipping_cost||0)||0); }
function baseOrderShippingCost(o){ return +(previousOrderShippingCost(o)+currentOrderShippingCost(o)).toFixed(2); }
...
function shippingCostCaption(o){
  const reships=Math.max(0,Number(o?.reship_count||0));
  if(reships>0) return `إجمالي ${reships+1} شحنات | الحالية ${fmtMoney(currentOrderShippingCost(o))}`;
  if(!isBostaOrder(o)) return baseOrderShippingCost(o)>0?'سعر شحن يدوي':'لم يتم تحديد شحن';
  const status=String(o?.status||'').trim();
  if(status==='تم التسليم') return 'شحن بوسطة + 1% المنتج + رسوم COD';
  if(status==='مرتجع') return 'مرتجع: الشحن فقط';
  return 'سعر الشحن محفوظ ويظهر فورًا';
}
...
// القص على مقاس الفورمة بيتزود 1.5 سم في الطول والعرض عشان فرق كسر السلاح — زي السيرفر بالظبط.
let FORM_CUT_ALLOWANCE_CM=1.5;
function withFormCutAllowance(v){const n=Number(v)||0;return n>0?+(n+FORM_CUT_ALLOWANCE_CM).toFixed(2):0}
function calcCutDims(o, mode){const formLength=Number(o.form_length)||0,formWidth=Number(o.form_width)||0,usesForm=Number(o.has_breaking||0)===1&&formLength>0&&formWidth>0;if(usesForm){const pieces=Math.max(1,Number(o.form_piece_count)||1),parts=o.form_construction_mode==='pieceByPiece'?2:1;return{cutWidth:withFormCutAllowance(formWidth),cutLength:withFormCutAllowance(formLength),piecesNeededPerBag:parts/pieces,usesFormCut:true}}const l=Number(o.l)||0,w=Number(o.w)||0,g=Number(o.g)||0; return {cutWidth: +(mode==='pieceByPiece' ? (w+g+2) : ((w*2)+(g*2)+2)).toFixed(2), cutLength:+(l+(g/2)+2).toFixed(2),piecesNeededPerBag:mode==='pieceByPiece'?2:1};}
...
function allowedStatusesForOrder(o){
  const list=Number(o?.useReadyStock||0)===1 ? READY_STOCK_ONLY_STATUSES : ALL_ORDER_STATUSES;
  return normalizedStatusText(o?.status)==='مرتجع' ? list.filter(status=>!['جاهز للشحن','تم الشحن','تم التسليم'].includes(status)) : list;
}
...
function buildDisplayOrders(rows){
  ...
  return list.map(entry=>{
    ...
    entry.reship_count=Math.max(...entry.group_items.map(item=>Number(item.reship_count||0)),0);
    entry.reship_carrier=entry.group_items.map(item=>String(item.reship_carrier||'').trim()).find(Boolean)||'';
...
function bostaInfoHtml(o){
  ...
  const previousShipping = previousOrderShippingCost(o);
  const currentShipping = currentOrderShippingCost(o);
  const grossSale = Math.max(0, numVal(o?.total_price, 0));
  const productValue = Math.max(0, numVal(o?.bosta_product_value, grossSale));
  const insurance = +(productValue * 0.01).toFixed(2);
  const extraCodFee = grossSale > 0 ? Math.ceil(grossSale / 1000) * 10 : 0;
  ...
  if(Number(previousShipping||0)>0) bits.push(`الشحنات السابقة: ${fmtMoney(previousShipping)}`);
  if(Number(currentShipping||0)>0) bits.push(`الشحنة الحالية: ${fmtMoney(currentShipping)}`);
  if(Number(previousShipping||0)>0&&Number(baseShipping||0)>0) bits.push(`إجمالي الشحنات: ${fmtMoney(baseShipping)}`);
  ...
  if(status==='تم الشحن' && Number(o?.reship_count||0)>0) bits.push('تكاليف كل محاولات الشحن اتخصمت بالفعل، حتى والشحنة الحالية لسه في الطريق.');
  else if(!['تم التسليم','مرتجع'].includes(status)) bits.push('الخصم الفعلي لا يتم إلا عند تم التسليم أو مرتجع.');
...
function reshipBadge(o){
  const count=Math.max(0,Number(o?.reship_count||0));
  if(!count) return '';
  const carrier=String(o?.reship_carrier||'').trim();
  return `<div class="tag reship-badge">🔁 إعادة شحن ${count>1?`${count} مرات`:'مرة واحدة'}${carrier?` | ${esc(carrier)}`:''}</div>`;
}
function reshipActionButton(o){
  const canReship=((user.username==='admin'||user.role==='super_admin')||Number(user.perm_edit_order||0));
  if(!canReship||normalizedStatusText(o?.status)!=='مرتجع') return '';
  return `<button class="btn green" onclick="openReshipModal(${Number(o.id)})">🔁 إعادة شحن</button>`;
}
function groupReshipActionButton(o){
  const canReship=((user.username==='admin'||user.role==='super_admin')||Number(user.perm_edit_order||0));
  const items=orderItems(o);
  if(!canReship||!items.length||items.some(item=>normalizedStatusText(item.status)!=='مرتجع')) return '';
  return `<button class="btn green" onclick="openGroupReshipModal(${jsAttrString(o.group_code)})">🔁 إعادة شحن الأوردر</button>`;
}
...
function singleOrderSizeHtml(o){
  return `${formatOrderSizeWithGram(o)}<br><small style="color:var(--muted)">${esc(normalizePaperType(o.paper_type))} | ${esc(o.color||'')}/${esc(o.handle||'')}${o.handle_type?`/${esc(o.handle_type)}`:''}${Number(o.has_lamination||0)?` | سلوفان ${esc(o.lamination_type||'')}`:''}${Number(o.has_breaking||0)?` | تكسير${o.form_id||o.form_code_snapshot?' - فورمة '+esc(o.form_code_snapshot||o.form_id)+' ('+(o.form_construction_mode==='pieceByPiece'?'حتة ف حتة':'حتة واحدة')+')':''}`:''}</small>`;
}
...
function renderSingleOrderRow(o,index,opts={}){
  ...
  return `<tr id="${rowId}" class="${Number(o.useReadyStock||0)===1?'ready-stock-row':''} ${isChild?'group-child-row group-child-'+groupKey:''}" style="${hiddenStyle}">
    ...
    <td><b style="font-size:16px;color:#fca5a5">${fmtMoney(orderShippingCost(o))}</b><br><small style="color:var(--muted)">${esc(shippingCostCaption(o))}</small></td>
    <td><b style="font-size:16px;color:#a5f3fc">${fmtMoney(totalCosts(o))}</b></td>
    <td>${paymentBadge(o)}</td>
    <td>${priorityBadge(o)}${reshipBadge(o)}<div style="height:6px"></div>${statusOptions(o)}${bostaInfoHtml(o)}</td>
...
function renderGroupOrderRow(o,index){
  ...
  const row=`<tr id="${rowDomIdForOrder(o)}" class="group-parent-row">
    ...
    <td>${paymentBadge(o)}</td>
    <td>${priorityBadge(o)}${reshipBadge(o)}<div style="height:6px"></div>${statusHtml}</td>
    <td class="files">${filesHtml(o)}</td>
    <td class="action-cell">
...
function renderOrders(data){
  ...
  body.innerHTML=data.length
    ? data.map((o,i)=>o.is_group ? renderGroupOrderRow(o, i+1) : renderSingleOrderRow(o, i+1)).join('')
    : '<tr><td class="orders-empty-cell" colspan="15"><strong>لا توجد أوردرات مطابقة</strong>غيّر الحالات المختارة أو راجع باقي الفلاتر لإظهار النتائج.</td></tr>';
...
function togglePanel(id){
  ...
  if(id==='filterPanel'&&!panel.classList.contains('open')) closeStatusFilterMenu();
}
function initStatusFilter(){
  const options=document.getElementById('statusFilterOptions');
  if(!options) return;
  options.innerHTML=ALL_ORDER_STATUSES.map(status=>{
    const meta=STATUS_META[status]||{};
    return `<label class="status-filter-option" data-status="${esc(status)}"><input type="checkbox" value="${esc(status)}" onchange="toggleStatusFilterValue(this)"><span class="status-filter-emoji">${meta.emoji||'📌'}</span><span class="status-filter-name">${esc(status)}</span></label>`;
  }).join('');
  syncStatusFilterUI();
}
function toggleStatusFilterMenu(button){
  const root=button?.closest('.status-multiselect');
  if(!root) return;
...
function closeStatusFilterMenu(){
  const root=document.getElementById('statusFilter');
  const button=document.getElementById('statusFilterButton');
  root?.classList.remove('open');
  button?.setAttribute('aria-expanded','false');
}
function syncStatusFilterUI(){
  document.querySelectorAll('#statusFilterOptions .status-filter-option').forEach(label=>{
    const input=label.querySelector('input');
    const checked=selectedStatusFilters.has(normalizedStatusText(input?.value||''));
    if(input) input.checked=checked;
    label.classList.toggle('selected',checked);
  });
  const count=selectedStatusFilters.size;
  const allSelected=count===ALL_ORDER_STATUSES.length;
  const text=document.getElementById('statusFilterText');
  const badge=document.getElementById('statusFilterCount');
  const hint=document.getElementById('statusFilterHint');
  if(text) text.textContent=allSelected?'كل الحالات':count===0?'بدون حالات':count===1?[...selectedStatusFilters][0]:`${count} حالات مختارة`;
  if(badge) badge.textContent=allSelected?'الكل':String(count);
  if(hint) hint.textContent=allSelected?'كل الحالات ظاهرة حاليًا.':count===0?'لن تظهر أوردرات حتى تختار حالة واحدة على الأقل.':`يتم عرض الأوردرات المطابقة لـ ${count} ${count===1?'حالة':'حالات'} مختارة.`;
}
function toggleStatusFilterValue(input){
  const status=normalizedStatusText(input?.value||'');
  if(!ALL_ORDER_STATUSES.includes(status)) return;
  if(input.checked) selectedStatusFilters.add(status);
  else selectedStatusFilters.delete(status);
  syncStatusFilterUI();
  applyFilters();
}
function setStatusFilterSelection(mode){
  selectedStatusFilters=mode==='all'?new Set(ALL_ORDER_STATUSES):new Set();
  syncStatusFilterUI();
  applyFilters();
}
...
function applyFilters(){
  ...
  const allStatusesSelected=selectedStatusFilters.size===ALL_ORDER_STATUSES.length;
  ...
  filteredOrders=buildDisplayOrders(ordersData).filter(o=>{
    const items=orderItems(o);
    // ملف مستخرج قديم غير مستخدم في الصفحة — التطبيق الفعلي داخل orders_list.html.
    // متساب متطابق معاه علشان ما يضلّلش أي حد يقرأه بعدين.
    const codeQuery=String(name||'').toUpperCase().replace(/[^A-Z0-9]/g,'');
    const codeHit=codeQuery&&String(o.store_ref||'').toUpperCase().replace(/[^A-Z0-9]/g,'').includes(codeQuery);
    const a=!name||(o.custName||'').toLowerCase().includes(name)||codeHit||String(o.order_display_no||'')===name.trim();
    const b=!phone||(o.custPhone||'').toLowerCase().includes(phone);
    const c=allStatusesSelected||items.some(item=>selectedStatusFilters.has(normalizedStatusText(item.status)));
...
function resetFilters(){ selectedStatusFilters=new Set(ALL_ORDER_STATUSES); syncStatusFilterUI(); closeStatusFilterMenu(); document.getElementById('paymentFilter').value=''; document.getElementById('printTypeFilter').value=''; document.getElementById('printSizeFilter').value=''; document.getElementById('createdByFilter').value=''; document.getElementById('pageFilter').value=''; document.getElementById('sourceFilter').value=''; document.getElementById('dateFrom').value=''; document.getElementById('dateTo').value=''; document.getElementById('priorityFilter').value=''; activeOnlyFilter=false; updateActiveOnlyButton(); applyFilters(); }
...
async function updateStatus(id,status){
  try{
    ...
    if(normalizedStatusText(order.status)==='مرتجع' && ['جاهز للشحن','تم الشحن','تم التسليم'].includes(status)){
      alert('استخدم زر «إعادة شحن» حتى تتسجل الشحنة الجديدة وتكلفتها بدون ضياع تكلفة الشحنة السابقة.');
      load();
      return;
    }
...
function updateEditStatusOptions(useReady,currentStatus='أوردر جديد'){
  ...
  if(current==='مرتجع') list=list.filter(s=>!['جاهز للشحن','تم الشحن','تم التسليم'].includes(s));
  if(current && !list.includes(current)) list=[current,...list];
...
function renderEditFilesList(files=[]){
  ...
  e_files_list.innerHTML = list.length
    ? list.map(f=>{ const id=Number(f.id); const name=esc(fileDisplayName(f)); return `<div class="edit-file-row"><span class="edit-file-name">📎 ${name}</span><div class="file-actions"><button type="button" class="btn dark file-delete-btn" onclick="openOrderFile(${id})">فتح</button><button type="button" class="btn cyan file-delete-btn" onclick="downloadOrderFile(${id}, ${jsAttrString(fileDisplayName(f))})">تحميل</button><button type="button" class="btn red file-delete-btn edit-file-remove" onclick="deleteOrderFile(${id})">🗑️ حذف الملف</button></div></div>`; }).join('')
    : 'لا توجد ملفات PDF';
}
function wholeGroupFinanceAmounts(items=[]){
  ...
  const shippingAll=sum(list, item=>currentOrderShippingCost(item));
  ...
  const shippingOther=sum(list.slice(1), item=>currentOrderShippingCost(item));
...
async function deleteOrderFile(fileId){
  ...
  try{
    ...
    alert('تم حذف ملف الـ PDF. اختَر الملف الجديد ثم اضغط حفظ التعديل.');
...
function normalizeBostaSearchText(v=''){
  return normalizeBostaText(v).toLowerCase()
...
function setBostaZoneSelection(selectedZone=''){
  ...
  if(match){ b_zone.value=match.value; return; }
  const allMatch=(currentBostaZoneOptions||[]).find(z=>{
    const code=normalizeBostaText(bostaOptionCode(z));
    const labels=[z?.label,...(z?.labels||[])].map(normalizeBostaText).filter(Boolean);
    return code===wanted || labels.some(label=>label===wanted || wanted.includes(label) || label.includes(wanted));
  });
  if(allMatch){
    const search=document.getElementById('b_zone_search');
    if(search) search.value='';
    renderBostaZoneOptions('',bostaOptionCode(allMatch));
    b_zone.value=bostaOptionCode(allMatch);
    return;
  }
  b_zone.value='';
}
function renderBostaZoneOptions(searchText='',selectedValue=''){
  const query=normalizeBostaSearchText(searchText);
  const selected=String(selectedValue||b_zone?.value||'').trim();
  const visible=(currentBostaZoneOptions||[]).filter(z=>{
    if(!query) return true;
    const text=[z?.label,z?.code,z?.id,...(z?.labels||[])].map(normalizeBostaSearchText).filter(Boolean).join(' ');
    return query.split(/\s+/).filter(Boolean).every(part=>text.includes(part));
  });
  b_zone.innerHTML='<option value="">اختر المنطقة</option>' + visible.map(z=>{const code=bostaOptionCode(z);const label=String(z?.label||code||'').trim();const fallback=Number(z?.isFallback||0)===1?'1':'0';const unavailable=z?.dropOffAvailable===false;return `<option value="${esc(code||label)}" data-label="${esc(label)}" data-fallback="${fallback}"${unavailable?' disabled':''}>${esc(label)}${unavailable?' — خارج التغطية':''}</option>`;}).join('');
  b_zone.size=query?Math.min(6,Math.max(2,visible.length+1)):1;
  if(selected && [...(b_zone.options||[])].some(opt=>String(opt.value||'')===selected)) b_zone.value=selected;
  const hint=document.getElementById('b_zone_search_hint');
  if(hint) hint.textContent=query ? (visible.length?`ظهر ${visible.length} نتيجة — افتح القائمة للاختيار.`:'لا توجد منطقة مطابقة للبحث.') : `${(currentBostaZoneOptions||[]).length} منطقة — اكتب للبحث أو افتح القائمة وانزل بالماوس.`;
}
function filterBostaZoneOptions(){
  const search=document.getElementById('b_zone_search');
  renderBostaZoneOptions(search?.value||'',b_zone?.value||'');
}
function onBostaZoneChange(){
  b_zone.size=1;
  renderBostaEstimateBox();
  requestBostaEstimate(true);
}
...
async function loadBostaZones(cityCode, selectedZone=''){
  currentBostaZoneOptions=[];
  const zoneSearch=document.getElementById('b_zone_search');
  ...
  try{
    ...
    if(Array.isArray(zones) && zones.length && zones.every(z=>Number(z?.isFallback||0)===1)){
      zones = await authFetch(`${url}&refresh=1`);
    }
  }catch(_){ zones = []; }
  ...
  if(!zones.length){
    ...
    const hint=document.getElementById('b_zone_search_hint');
    if(hint) hint.textContent='لا توجد مناطق متاحة لهذه المحافظة الآن.';
    return;
  }
  currentBostaZoneOptions=zones;
  renderBostaZoneOptions('',selectedZone);
...
function toggleReshipMethod(){
  const mode=String(document.getElementById('reship_method')?.value||'manual');
  const manual=mode==='manual';
  document.getElementById('reship_carrier_wrap')?.classList.toggle('hidden',!manual);
  document.getElementById('reship_cost_wrap')?.classList.toggle('hidden',!manual);
  const hint=document.getElementById('reshipMethodHint');
  const button=document.getElementById('reshipSubmitBtn');
  if(hint) hint.textContent=manual?'اكتب شركة الشحن وتكلفة الشحنة الجديدة. التكلفة القديمة ستظل محفوظة والجديدة ستُضاف عليها.':'سيتم فتح بيانات بوسطة وإرسال بوليصة جديدة، مع الاحتفاظ بتكلفة الشحنة السابقة.';
  if(button) button.textContent=manual?'🔁 تأكيد إعادة الشحن':'🚚 متابعة مع بوسطة';
}
function openReshipModal(id){
  const order=ordersData.find(row=>Number(row.id)===Number(id));
  if(!order) return alert('الأوردر غير موجود');
  if(normalizedStatusText(order.status)!=='مرتجع') return alert('إعادة الشحن متاحة للأوردر المرتجع فقط');
  currentReshipOrderId=Number(id);
  currentReshipGroupCode='';
  const previousTotal=baseOrderShippingCost(order);
  const completedAttempts=Math.max(1,Number(order.reship_count||0)+1);
  const summary=document.getElementById('reshipSummary');
  if(summary) summary.innerHTML=`<b>الأوردر #${Number(order.order_display_no||order.id)}</b> — ${esc(order.custName||'')}<br>الشحنات السابقة: <b>${completedAttempts}</b> | تكلفتها المحفوظة: <b>${fmtMoney(previousTotal)}</b><br><small>الشحنة الجديدة هتتسجل كشحنة رقم ${completedAttempts+1}، وتكلفتها هتتضاف على القديم.</small>`;
  document.getElementById('reship_method').value='manual';
  document.getElementById('reship_carrier').value='';
  document.getElementById('reship_cost').value='';
  document.getElementById('reship_notes').value='';
  toggleReshipMethod();
  document.getElementById('reshipModal').style.display='block';
}
function openGroupReshipModal(groupCode){
  const code=String(groupCode||'').trim();
  const items=getGroupItemsByCode(code);
  const order=buildGroupBostaContext(code);
  if(!order||!items.length) return alert('الأوردر المجمّع غير موجود');
  if(items.some(item=>normalizedStatusText(item.status)!=='مرتجع')) return alert('إعادة الشحن متاحة لما تكون كل أصناف الأوردر مرتجع');
  currentReshipOrderId=0;
  currentReshipGroupCode=code;
  const previousTotal=items.reduce((sum,item)=>sum+baseOrderShippingCost(item),0);
  const completedAttempts=Math.max(1,...items.map(item=>Number(item.reship_count||0)+1));
  const summary=document.getElementById('reshipSummary');
  if(summary) summary.innerHTML=`<b>الأوردر المجمّع ${esc(code)}</b> — ${esc(order.custName||'')}<br>عدد الأصناف: <b>${items.length}</b> | الشحنات السابقة: <b>${completedAttempts}</b> | تكلفتها المحفوظة: <b>${fmtMoney(previousTotal)}</b><br><small>الشحنة الجديدة هتتسجل كبوليصة واحدة لكل أصناف الأوردر، وتكلفتها هتتضاف على القديم مرة واحدة.</small>`;
  document.getElementById('reship_method').value='manual';
  document.getElementById('reship_carrier').value='';
  document.getElementById('reship_cost').value='';
  document.getElementById('reship_notes').value='';
...
async function submitReship(){
  const groupCode=String(currentReshipGroupCode||'').trim();
  const order=groupCode ? buildGroupBostaContext(groupCode) : ordersData.find(row=>Number(row.id)===Number(currentReshipOrderId));
  if(!order) return alert('الأوردر غير موجود');
  const mode=String(document.getElementById('reship_method')?.value||'manual');
  if(mode==='bosta'){
    closeModal('reshipModal');
    await openBostaModalByContext(order,{orderId:Number(order.id),groupCode,isReship:true});
    return;
  }
  const carrier=String(document.getElementById('reship_carrier')?.value||'').trim();
  const shippingCost=Math.max(0,Number(document.getElementById('reship_cost')?.value||0));
  const notes=String(document.getElementById('reship_notes')?.value||'').trim();
  if(!carrier) return alert('اكتب اسم شركة الشحن');
  if(shippingCost<=0) return alert('اكتب تكلفة الشحنة الجديدة');
  try{
    const endpoint=groupCode?`/reship-order-group/${encodeURIComponent(groupCode)}`:`/reship-order/${Number(order.id)}`;
    const data=await authFetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({carrier,shipping_cost:shippingCost,notes})});
    alert(`تمت إعادة الشحن بنجاح مع ${carrier}\nتكلفة الشحنة الجديدة: ${fmtMoney(data.shipping_cost)}\nإجمالي الشحنات: ${fmtMoney(data.total_shipping_cost)}`);
    closeModal('reshipModal');
    await load();
  }catch(e){ alert(e.message||'تعذر إعادة الشحن'); }
}
async function openBostaModalByContext(order,opts={}){
  ...
  window.currentBostaIsReship=!!opts.isReship;
  if(window.bostaModalTitle) bostaModalTitle.textContent=window.currentBostaIsReship?'🔁 إعادة الشحن مع بوسطة':'🚚 إرسال إلى بوسطة';
  ...
  window.currentBostaProductManual = false;
  syncBostaProductValue(true);
  const nextReshipNo=Math.max(1,Number(order.reship_count||0)+1);
  b_business_reference.value=window.currentBostaIsReship
    ? `${order.is_group?'GROUP-'+String(order.group_code||order.id||'').trim():'ORDER-'+order.id}-R${nextReshipNo}-${String(Date.now()).slice(-6)}`
    : (order.bosta_business_reference||(order.is_group?String(order.group_code||'').trim():`ORDER-${order.id}`));
  ...
  try{
    guessed = await authFetch('/bosta-infer-location',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({address:b_second_line.value||order.custAddress||'',city_code:preferredCityToken || order.customer_governorate || order.bosta_city_code || '',zone:order.customer_zone||order.bosta_zone||'',prefer_address:1})});
  }catch(_){ }
  const selectedCityRaw = guessed.cityLabel || guessed.cityCode || order.customer_governorate || order.bosta_city_code || '';
  const selectedCity = normalizeBostaCityToken(selectedCityRaw) || selectedCityRaw;
  const selectedZone = guessed.zone || guessed.zoneLabel || order.customer_zone || order.bosta_zone_label || order.bosta_zone || order.bosta_zone_code || '';
  try{
    ...
    if(!b_zone.value && (guessed.zone||guessed.zoneLabel)) setBostaZoneSelection(guessed.zone||guessed.zoneLabel);
  }catch(e){
  ...
  const chosenZoneText=bostaSelectedZoneLabel() || guessed.zoneLabel || selectedZone || '-';
  if(guessed.source==='address' && b_zone.value) autoBits.push(`✅ تم تحديد المحافظة والمنطقة تلقائيًا من العنوان: ${cityText} / ${chosenZoneText}`);
  else if(selectedCity || selectedZone) autoBits.push(`الاختيار الحالي: ${cityText} / ${chosenZoneText}`);
  autoBits.push(order.is_group ? 'الأوردر ده مجمع وبيتبعت كبوليصة واحدة إلى بوسطة. قيمة المنتج والـ COD محسوبين تلقائيًا على إجمالي الأوردر، وكل البيانات تفضل قابلة للتعديل قبل الإرسال.' : 'النظام يحاول تحديد المحافظة والمنطقة من العنوان تلقائيًا، وتقدر تعدل أي اختيار قبل الإرسال. قيمة المنتج تتملأ من إجمالي الأوردر وتفضل قابلة للتعديل.');
...
async function sendToBosta(){
  ...
  const body={
    ...
    notes:b_notes.value,
    reship:window.currentBostaIsReship?1:0
  };
  try{
    ...
    const parts=[data.reship ? 'تمت إعادة الشحن مع بوسطة بنجاح' : (groupCode ? 'تم إرسال الأوردر المجمّع إلى بوسطة بنجاح' : 'تم الإرسال إلى بوسطة بنجاح')];
    ...
    if(data.reship&&data.totalShippingCost) parts.push(`إجمالي الشحنات حتى الآن: ${fmtMoney(data.totalShippingCost)}`);
    alert(parts.join('\n'));
...
function closeModal(id){ document.getElementById(id).style.display='none'; if(id==='detailsModal'){detailsChoices.innerHTML=''; detailsBox.innerHTML=''; saveCutBtn.style.display='none'; window.currentPlan=null; window.pendingTargetStatus=null; if(!window.skipGroupFlowReset) currentGroupStatusFlow=null;} if(id==='bostaModal'){ window.currentBostaOrderId=null; window.currentBostaGroupCode=null; window.currentBostaIsReship=false; window.currentBostaAllowOpen=false; window.currentBostaEstimate=null; window.currentBostaEstimateKey=''; window.currentBostaEstimateLoading=false; window.currentBostaShowDetails=false; if(window.bostaModalTitle)bostaModalTitle.textContent='🚚 إرسال إلى بوسطة'; } if(id==='reshipModal'){ currentReshipOrderId=0; currentReshipGroupCode=''; } if(id==='paymentsModal'){ currentPaymentsOrderId=null; } if(id==='directionModal'){ currentDirectionOrderId=null; currentDirectionData={order:null,steps:[],partners:[]}; } if(id==='printChoiceModal'){ printSelectionSnapshot=[]; } if(id==='orderFinanceModal'){ currentOrderFinanceId=0; orderFinanceBox.innerHTML=''; } }
...
window.openGroupReshipModal = openGroupReshipModal;
window.submitReship = submitReship;
window.toggleReshipMethod = toggleReshipMethod;
window.delOrder = delOrder;
window.toggleActions = toggleActions;
window.closeModal = closeModal;
document.addEventListener('click',e=>{ if(!e.target.closest('.action-cell')) closeAllActionMenus(); if(!e.target.closest('.status-multiselect')) closeStatusFilterMenu(); });
document.addEventListener('keydown',e=>{ if(e.key==='Escape') closeStatusFilterMenu(); });
initStatusFilter();
load();
