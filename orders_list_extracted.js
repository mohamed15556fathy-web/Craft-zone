...
function buildAutoCosts(o){
  ...
  const out={cut:manual('cut',50), print:manual('print',0), zinc:manual('zinc',0), design:manual('design',0), make:manual('make',qty*1.2), hand:manual('hand',0), paper:manual('paper',0), hand_fix:manual('hand_fix',(String(o.handle||'')==='بيد'? +((qty/1000)*100).toFixed(2):0))};
...
function collectDirectionPayload(prefix, stepType){
  ...
  if(!partnerId && !paidNow) return null;
