...
function buildBostaPayload(order, extra={}, req=null) {
  ...
  const codValue = Math.max(0, num(extra.cod, order?.bosta_cod || order?.remaining_amount || 0));
  const productValue = Math.max(0, num(extra.product_value, codValue || order?.bosta_product_value || order?.total_price || order?.remaining_amount || 0));
...
app.post('/send-to-bosta/:id', authRequired, requirePerm('perm_edit_order'), async (req, res) => {
  try {
    ...
    const selectedZoneToken = String(body.zone || customerZone || order.bosta_zone || '').trim();
    ...
    const productValue = Math.max(0, num(body.product_value, codAmountInput || order.bosta_product_value || orderSaleValue || order.total_price || 0));
    ...
    const location = await resolveBostaShipmentLocation({
      ...
      zoneToken: selectedZoneToken || customerZone || order.bosta_zone || ''
    });
...
app.post('/send-to-bosta-group/:groupCode', authRequired, requirePerm('perm_edit_order'), async (req, res) => {
  try {
    ...
    const selectedCityLabel = String(body.city_label || resolveConfiguredBostaGovernorateLabel(selectedCityToken, bostaCities) || customerGovernorate || '').trim();
    const selectedZoneToken = String(body.zone || customerZone || baseOrder.bosta_zone || '').trim();
    ...
    const productValue = Math.max(0, num(body.product_value, codAmountInput || baseOrder.bosta_product_value || groupSaleValue || baseOrder.total_price || 0));
    ...
    const location = await resolveBostaShipmentLocation({
      ...
      cityLabel: selectedCityLabel || customerGovernorate || '',
      zoneToken: selectedZoneToken || customerZone || baseOrder.bosta_zone || ''
    });
    const cityCode = String(location.cityCode || '').trim();
    const zone = String(location.zone || '').trim();
