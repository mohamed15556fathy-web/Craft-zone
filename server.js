...
// Public health endpoint for Railway/load balancer: returns fast without auth or database work.
app.get('/healthz', (req, res) => res.status(200).json({ ok: true, service: 'Craft-zone', time: new Date().toISOString() }));
...
function ensureBostaConfigured() {
  if (!BOSTA_ENABLED) {
    const err = new Error('BOSTA_API_KEY غير مضبوط. أضفه في Environment Variables أو ملف .env محلي غير مرفوع');
...
function isSafeUploadFile(file = {}, { backupOnly = false } = {}) {
  const original = String(file.originalname || '').trim();
  const ext = path.extname(original).toLowerCase();
  if (backupOnly) return ['.zip', '.db', '.sqlite', '.sqlite3'].includes(ext);
  const allowedExts = new Set(['.pdf','.png','.jpg','.jpeg','.webp','.gif','.svg','.doc','.docx','.xls','.xlsx','.csv','.txt','.zip']);
  if (!allowedExts.has(ext)) return false;
  const blocked = new Set(['.exe','.bat','.cmd','.sh','.ps1','.js','.mjs','.cjs','.html','.php','.py','.rb','.jar']);
  return !blocked.has(ext);
}
function makeSafeMulter(dest, { maxFileSizeMb = 15, backupOnly = false } = {}) {
  return multer({
    dest,
    limits: { fileSize: maxFileSizeMb * 1024 * 1024, files: backupOnly ? 1 : 20 },
    fileFilter: (req, file, cb) => {
      if (!isSafeUploadFile(file, { backupOnly })) return cb(new Error(backupOnly ? 'صيغة النسخة الاحتياطية غير مدعومة' : 'نوع الملف غير مسموح'));
      cb(null, true);
    }
  });
}
...
const upload = makeSafeMulter(UPLOAD_DIR, { maxFileSizeMb: 25 });
const backupImportUpload = makeSafeMulter(BACKUP_DIR, { maxFileSizeMb: 250, backupOnly: true });
...
function createMemoryRateLimiter({ windowMs = 60_000, max = 60, message = 'محاولات كثيرة، جرّب بعد دقيقة' } = {}) {
  const hits = new Map();
  return (req, res, next) => {
    ...
    row.count += 1;
    hits.set(key, row);
    if (row.count > max) return res.status(429).json({ error: message });
    next();
  };
}
app.use('/login', createMemoryRateLimiter({ windowMs: 15 * 60 * 1000, max: 20, message: 'محاولات تسجيل دخول كثيرة، جرّب بعد ربع ساعة' }));
app.use(['/import-backup','/backup-db','/backup-db-file','/system-backup-now'], createMemoryRateLimiter({ windowMs: 60 * 1000, max: 8 }));
...
function sendRootSafeFile(res, absolutePath) {
  ...
  const forceFresh = ext === '.html' || path.basename(String(absolutePath || '')).toLowerCase() === 'cutting_optimizer.js';
  const opts = forceFresh ? { maxAge: 0, lastModified: true } : { maxAge: '1h', etag: true, lastModified: true };
  if (forceFresh) res.setHeader('Cache-Control', 'no-store, max-age=0');
...
async function seedDefaultAppSettings() {
  const defaults = [
    ...
    ['auto_backup_enabled', getCachedSetting('auto_backup_enabled', '1'), 'تفعيل نسخة احتياطية تلقائية يوميًا', 'النسخ الاحتياطي'],
    ['auto_backup_keep_count', getCachedSetting('auto_backup_keep_count', '14'), 'عدد نسخ قاعدة البيانات التي يتم الاحتفاظ بها', 'النسخ الاحتياطي'],
    ['last_auto_backup_at', getCachedSetting('last_auto_backup_at', ''), 'آخر نسخة احتياطية تلقائية', 'النسخ الاحتياطي']
  ];
...
async function firstBlockingAuditLog(logRow) {
  const refs = normalizeTouchRefs(logRow?.touch_refs || '[]', logRow?.entity_type || '', logRow?.entity_id || null);
  ...
  for (const row of laterRows) {
    const otherRefs = normalizeTouchRefs(row?.touch_refs || '[]', row?.entity_type || '', row?.entity_id || null);
    if (otherRefs.some(ref => refs.includes(ref))) return row;
  }
  return null;
}
function auditTouchRefsForRow(row = {}) {
  return normalizeTouchRefs(row?.touch_refs || '[]', row?.entity_type || '', row?.entity_id || null);
}
function touchRefsOverlap(a = [], b = []) {
  const set = new Set(uniqueList(a));
  return uniqueList(b).some(ref => set.has(ref));
}
async function collectDependentAuditLogs(rootLog = {}) {
  const rootId = num(rootLog?.id);
  if (!rootId) return [];
  const rows = await allAsync(`SELECT * FROM audit_logs WHERE id >= ? AND COALESCE(reverted_at,'') = '' ORDER BY id ASC`, [rootId]);
  const selected = [];
  const selectedIds = new Set();
  const refs = new Set(auditTouchRefsForRow(rootLog));
  for (const row of rows) {
    const rowRefs = auditTouchRefsForRow(row);
    const isRoot = num(row.id) === rootId;
    const related = isRoot || (rowRefs.length && touchRefsOverlap([...refs], rowRefs));
    if (!related) continue;
    if (!selectedIds.has(num(row.id))) {
      selected.push(row);
      selectedIds.add(num(row.id));
    }
    for (const ref of rowRefs) refs.add(ref);
  }
  return selected;
}
function num(v, d = 0) {
...
async function buildAdminCashSourceLabel(row = {}) {
  ...
  try {
    ...
    if (sourceType === 'purchase_payment') {
      const p = await getAsync(`SELECT pp.*, pu.supplier_name, pu.item_name, pu.invoice_group_no, pu.id purchase_no
        FROM purchase_payments pp LEFT JOIN purchases pu ON pu.id=pp.purchase_id
        WHERE CAST(pp.id AS TEXT)=CAST(? AS TEXT) LIMIT 1`, [sourceRef]);
      if (p) {
        const invoiceNo = purchaseGroupLabelForRow(p) || String(num(p.purchase_id));
        const supplier = String(p.supplier_name || '').trim();
        return `دفعة مشتريات - فاتورة #${invoiceNo}${supplier ? ' - '+supplier : ''}`;
      }
      return note ? `دفعة مشتريات - ${note.replace(/^خرج:\s*/, '')}` : 'دفعة مشتريات';
    }
...
async function cleanupUndueOrderCostExpenses({ reason = 'auto-migration' } = {}) {
  ...
  for (const row of rows) {
    ...
    // لو الأوردر اتحذف أو نوع التكلفة غير معروف أو تكلفة مخزون اتسجلت كمصروف خزنة، احذفها.
    // الورق واليد تكلفتهم على الأوردر وسجل المخزن فقط، ولا ينفع يتخصموا من الخزنة كمصروف أوردر.
    const orderMissing = !num(row.id);
    const invalidField = !ORDER_COST_ALL_FIELDS.includes(field);
    const stockCostInCash = ORDER_COST_STOCK_FIELDS.includes(field);
    const due = !orderMissing && !invalidField && !stockCostInCash && orderCostFieldDue(row, field);
    if (due) continue;
...
async function cleanupDuplicateOrderCostExpenses({ reason = 'auto-migration' } = {}) {
  ...
  for (const [key, groupRows] of groups.entries()) {
    ...
    const normalizedExpenseDate = await getOrderCostAccountingDate(order, field);
    await runAsync(`UPDATE expenses SET expense_date=?,amount=?,category='تكاليف أوردر',custom_category='',notes=?,linked_to_order=1,order_id=?,order_cost_field=?,expense_partner_name=?,actor_username=?,actor_name=?,execution_partner_id=?,execution_partner_name=?,execution_partner_type=?,source_type=?,source_ref=?,is_auto=1 WHERE id=?`, [normalizedExpenseDate, amount, note, orderId, field, partnerName, actorUsername, actorName, num(op?.partner_id || keep.execution_partner_id || 0), String(op?.partner_name || keep.execution_partner_name || '').trim(), String(op?.partner_type || keep.execution_partner_type || '').trim(), sourceType, sourceRef, expenseId]);
    await deletePartnerFundEntriesBySource('expense', String(expenseId));
    await deleteAdminCashEntriesBySource('expense', String(expenseId));
    if (partnerName) await syncExpensePartnerFund({ expenseId, partnerName, amount, expenseDate: normalizedExpenseDate, linkedToOrder: 1, notes: note, createdBy: reason || 'system' });
    else await syncExpenseAdminCash({ expenseId, actorUsername, actorName, amount, expenseDate: normalizedExpenseDate, linkedToOrder: 1, notes: note, createdBy: reason || 'system' });
    await runAsync(`DELETE FROM cost_history WHERE source='accounts' AND source_ref=?`, [String(expenseId)]);
    await addCostLog({ order_id: orderId, cost_field: field, amount, source:'accounts', source_ref:String(expenseId), notes: note, created_by: reason || 'system', cost_date: normalizedExpenseDate });
    normalized++;
  }
...
function normalizePhone(phone='') {
  let value = normalizeNumericText(phone);
  if (!value) return '';
  value = value.replace(/[^\d+]/g, '');
  if (value.startsWith('00')) value = '+' + value.slice(2);
  return value;
}
const ORDER_PAGE_NAMES = Object.freeze(['El7amd Factory - مصنع الحمد','Craft Zone','Craft House - بــيت الكرافت']);
function normalizeOrderPageName(value='') {
  const clean = String(value || '').trim().replace(/\s+/g, ' ');
  return ORDER_PAGE_NAMES.includes(clean) ? clean : '';
}
...
async function ensureProfessionalSchema() {
  ...
  const uniqueIndexes = [
    ...
    [`CREATE UNIQUE INDEX IF NOT EXISTS ux_bag_sub_order_once ON bags_history(order_id, type) WHERE COALESCE(order_id,0)>0 AND TRIM(COALESCE(type,''))='sub'`, 'unique_bag_sub_order_once'],
...
async function ensureSchema() {
  ...
  for (const jobs of legacyGroups.values()) {
    ...
    const inserted = await runAsync(`INSERT INTO cutting_batches (created_at,paper_id,paper_label,sheet_length,sheet_width,grammage,color,paper_type,total_sheets,total_kg,waste_percent,production_jobs_count,returned_cut_pieces,returned_cut_kg,items_json,plan_json,note,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [
      first.production_date || new Date().toISOString(), paperId, paperLabelFull(paperRow || {}), num(paperRow?.length), num(paperRow?.width), num(paperRow?.grammage), String(paperRow?.color || first.color || '').trim(), paperTypeFromRow(paperRow || {}), totalSheets, totalKg, 0, jobs.length, returnedPieces, returnedKg, JSON.stringify(items), JSON.stringify(legacyPlan), historyNote || 'دفعة مخطط قص قديمة تم تجميعها تلقائيًا', String(first.created_by || 'system').trim()
    ]);
...
async function applyWholeGroupFinancials(orderId, { total = 0, paid = 0, paymentType = '', shippingCost = 0, createdBy = 'system', actorUsername = '' } = {}) {
  ...
  const groupPaid = roundMoney(Math.max(0, Math.min(groupTotal, num(state.paid_amount))));
  const groupRemaining = roundMoney(Math.max(0, groupTotal - groupPaid));
...
async function normalizeAllWholeGroupFinancialStorage(createdBy = 'system') {
  ...
  for (const g of groups || []) {
    ...
    const currentAlreadyNormalized =
      ...
      roundMoney(primaryTotal) === groupTotal && roundMoney(primaryPaid) === groupPaid;
...
async function deductReadyBagStockIfNeeded(order = {}, note = '', opts = {}) {
  ...
  await runAsync(`INSERT INTO bags_history (bag_id,order_id,type,qty,date,color,handle,length,width,gusset,note) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [info.bag.id, num(order.id), 'sub', info.required, eventDate, info.bag.color, info.bag.handle, info.bag.length, info.bag.width, info.bag.gusset, note || `خصم من مخزن الشنط عند تجهيز الأوردر #${num(order.id)} للشحن`]);
...
async function estimatePaperCostFromRecordedHistory(order = {}) {
  ...
  const rows = await allAsync(`SELECT ph.id, ph.paper_id, ph.kg, ph.sheets, ph.note, p.buy_price_kg, p.length, p.width, p.grammage, p.paper_name, p.paper_type, p.color
    FROM paper_history ph
    LEFT JOIN paper p ON p.id=ph.paper_id
    WHERE TRIM(COALESCE(ph.type,''))='sub' AND TRIM(COALESCE(ph.note,'')) LIKE ?
    ORDER BY ph.id ASC LIMIT 1000`, [`%#${orderId}%`]).catch(()=>[]);
  const matched = (rows || []).filter(row => inventoryHistoryNoteMentionsOrder(row.note, orderId));
  let cost = 0;
  let kgTotal = 0;
  let sheetsTotal = 0;
  ...
  const logged = await getAsync(`SELECT COALESCE(SUM(amount),0) total, COUNT(*) c FROM cost_history WHERE order_id=? AND cost_field='cost_paper' AND COALESCE(amount,0)>0`, [orderId]).catch(()=>null);
  const loggedCost = roundMoney(num(logged?.total));
  if (loggedCost > 0) return { cost: loggedCost, source: 'سجل التكاليف', rows: num(logged?.c), exact: 0 };
...
async function syncAllMissingPaperCostsFromHistories({ reason = 'system', repairMismatch = true } = {}) {
...
async function repairMissingCalculatedOrderCostsForOrder(orderId = 0, { actor='system', repairMismatch=true } = {}) {
  const cleanOrderId = num(orderId);
  ...
  for (const field of ORDER_COST_ALL_FIELDS) {
    try {
      ...
      const isMismatchRepair = field === 'cost_paper' && repairMismatch && currentAmount > 0 && Math.abs(currentAmount - expectedAmount) > 0.5;
...
async function syncAllMissingCalculatedOrderCosts({ reason='system', repairMismatch=true } = {}) {
...
const ORDER_COST_STOCK_FIELDS = ['cost_paper','cost_hand'];
...
async function calcSalesDeductionsForOrder(order = {}, status = '') {
  ...
  const direct = cleanStatus === 'مرتجع'
    ? { shippingFee: getEffectiveOrderShippingFee(order), insuranceFee: 0, extraCodFee: 0, totalDeductions: getEffectiveOrderShippingFee(order) }
...
async function resyncBostaShipmentSalesSiblings(order = {}, created_by = '') {
  const siblings = await getBostaShipmentOrders(order, ['تم التسليم', 'مرتجع']);
...
async function syncSaleForOrder(order, created_by='', syncSiblings=true) {
  ...
  if (status === 'تم التسليم') {
    ...
    const printedReturnLoss = isPrintedOrder(order.printType) ? calcOrderTotalCosts(order) : 0;
    total_cost = roundMoney(printedReturnLoss);
    net_profit = roundMoney(total_sale - total_cost);
    paid_amount = 0;
    remaining_amount = 0;
    notes = printedReturnLoss > 0 ? 'أوردر مرتجع مطبوع - خسارة إنتاج + خصم شحن' : 'أوردر مرتجع - خصم الشحن فقط';
  }
...
async function cleanupOrphanAutoFinancialLedgers() {
  ...
  await runAsync(`DELETE FROM admin_cash_ledger WHERE source_type='purchase_payment' AND NOT EXISTS (SELECT 1 FROM purchase_payments p WHERE CAST(p.id AS TEXT)=CAST(admin_cash_ledger.source_ref AS TEXT))`);
...
async function buildOrderFinanceSummary(orderId = 0) {
  ...
  const labels = {
    ...
    cost_forme: 'فورمة', cost_lamination: 'سلوفان', cost_make: 'تصنيع', cost_hand: 'يد', cost_hand_fix: 'تركيب يد'
  };
  const fields = ['cost_paper','cost_cut','cost_zinc','cost_design','cost_print','cost_forme','cost_lamination','cost_make','cost_hand','cost_hand_fix'];
...
async function paperHistoryRowsForOrder(orderId=0, paperId=0) {
  const cleanOrderId = num(orderId);
  const rows = await allAsync(`SELECT * FROM paper_history WHERE TRIM(COALESCE(type,''))='sub' AND (COALESCE(order_id,0)=? OR TRIM(COALESCE(note,''))<>'') ORDER BY id ASC`, [cleanOrderId]).catch(()=>[]);
  return (rows || []).filter(h => (!num(paperId) || num(h.paper_id) === num(paperId)) && (num(h.order_id) === cleanOrderId || inventoryHistoryNoteMentionsOrder(h.note, cleanOrderId)));
}
...
async function repairPaperInventoryForOrder(order = {}, { actor='system', forceHistoricalStock=false } = {}) {
  ...
  if (num(order.paper_cut_done) === 1 && paperId && paper && !existing.length) {
    ...
    if (kg > 0 || sheets > 0) {
      ...
      await runAsync(`INSERT INTO paper_history (paper_id,type,kg,sheets,date,color,length,width,grammage,paper_name,paper_type,note) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`, [paperId, 'sub', -kg, -sheets, stageDate, paper.color || '', num(paper.length), num(paper.width), num(paper.grammage), paper.paper_name || '', paperTypeFromRow(paper), note]);
...
async function repairHandleInventoryForOrder(order = {}, { actor='system', forceHistoricalStock=false } = {}) {
  ...
  if (num(order.handle_stock_deducted) === 1 && !hasHistory) {
    ...
    await runAsync(`INSERT INTO handles_history (handle_id,order_id,type,qty,color,handle_type,note,date) VALUES (?,?,?,?,?,?,?,?)`, [num(h?.id), orderId, 'sub', num(order.qty), String(h?.color || order.color || '').trim(), handleType, note, stageDate]);
  ...
  if (!num(order.handle_stock_deducted) && isStatusAtLeastForRepair(order, 'جاهز للشحن')) {
    ...
    if (!handleRow && forceHistoricalStock) {
      const ins = await runAsync(`INSERT INTO handles (color,handle_type,qty,buy_price,min_qty) VALUES (?,?,?,?,?)`, [String(order.color || '').trim(), handleType, 0, 0, 0]);
  ...
  if (shouldDeduct && !num(order.ready_stock_deducted) && !hasHistory) {
    try {
      if (forceHistoricalStock) {
        ...
        await runAsync(`INSERT INTO bags_history (bag_id,order_id,type,qty,date,color,handle,length,width,gusset,note) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [num(bag.id), orderId, 'sub', requiredQty, stageDate, String(bag.color || order.color || '').trim(), String(bag.handle || order.handle || '').trim(), num(bag.length || order.l), num(bag.width || order.w), num(bag.gusset || order.g), `خصم شنط جاهزة تلقائي من فحص السيستم للأوردر #${orderId}`]);
        await runAsync(`UPDATE orders SET ready_stock_deducted=1, bag_returned_to_stock=0 WHERE id=?`, [orderId]);
...
async function runSystemHealthCheck() {
  ...
  const labelForOrder = async (orderId, custName='') => {
    ...
    if (!labelCache.has(key)) labelCache.set(key, await orderDisplayLabelForId(id, custName || ''));
  ...
  const paidRows = await allAsync(`SELECT e.*, o.id AS order_exists, o.status AS order_status, o.custName AS custName, o.printType AS order_printType, o.handle AS order_handle, o.useReadyStock AS order_useReadyStock, o.paper_cut_done AS order_paper_cut_done, o.handle_stock_deducted AS order_handle_stock_deducted, ${orderCostFields.map(f=>`o.${f} AS order_${f}`).join(', ')} FROM expenses e LEFT JOIN orders o ON o.id=e.order_id WHERE COALESCE(e.linked_to_order,0)=1 AND TRIM(COALESCE(e.order_cost_field,''))<>''`);
  ...
  const orders = await allAsync(`SELECT * FROM orders ORDER BY id DESC LIMIT 2000`);
  ...
  for (const o of orders) {
    ...
    for (const field of orderCostFields) {
      ...
      } else if (field === 'cost_paper' && expectedAmount > 0 && currentAmount > 0 && Math.abs(expectedAmount - currentAmount) > 0.5) {
        push('warn', 'فرق في تكلفة الورق المسجلة', `${label} | المسجل على الأوردر ${currentAmount.toFixed(2)} ج / المتوقع من السجل ${expectedAmount.toFixed(2)} ج | المصدر: ${expected.source || '-'}`, { order_id:num(o.id), order_label:label, field, expected:expectedAmount, current:currentAmount });
      }
    }
  }

  const inventoryOrders = await allAsync(`SELECT * FROM orders WHERE TRIM(COALESCE(status,'')) NOT IN ('مرتجع','ملغي') ORDER BY id DESC LIMIT 3000`);
  ...
  const paperHistoryForOrder = (orderId=0, paperId=0) => paperInventoryHistoryRows.filter(h => (!paperId || num(h.paper_id) === num(paperId)) && (num(h.order_id) === num(orderId) || inventoryHistoryNoteMentionsOrder(h.note, orderId)));
  ...
  for (const o of inventoryOrders) {
    ...
    if (!ready) {
      ...
      if (activeStatusAtLeast(o, 'في القص') && !num(o.paper_cut_done)) {
        if (paperHist.length) push('warn', 'خصم ورق موجود بالسجل لكن الأوردر غير محدّث', `${label} | توجد حركة خصم ورق في سجل المخزن لكن علامة خصم الورق على الأوردر غير مفعلة`, { order_id:num(o.id), order_label:label, history_count:paperHist.length });
        else push('warn', 'خصم ورق غير منفذ من المخزن', `${label} | الحالة ${o.status || '-'} وصلت لمرحلة القص أو بعدها لكن الورق غير مخصوم من مخزن الورق`, { order_id:num(o.id), order_label:label });
      }
      if (num(o.paper_cut_done)) {
        const paperId = num(o.last_cut_paper_id);
        if (!paperId) push('error', 'خصم ورق بدون تحديد صنف الورق', `${label} | الأوردر متعلم عليه أنه اتقص لكن لا يوجد last_cut_paper_id`, { order_id:num(o.id), order_label:label });
        else if (!existingPaperIds.has(paperId)) push('error', 'خصم ورق مرتبط بصنف محذوف', `${label} | صنف الورق #${paperId} غير موجود في مخزن الورق`, { order_id:num(o.id), order_label:label, paper_id:paperId });
        if (paperId && paperHist.length === 0) push('warn', 'خصم ورق غير موجود في سجل المخزن', `${label} | الأوردر متعلم عليه خصم ورق من الصنف #${paperId} لكن لا توجد حركة خصم مقابلة في سجل الورق`, { order_id:num(o.id), order_label:label, paper_id:paperId });
      }
  ...
  for (const h of orphanHandleHistory) push('error', 'سجل مخزن يد مرتبط بأوردر محذوف', `حركة يد #${num(h.id)} مرتبطة بأوردر غير موجود #${num(h.order_id)}`, { history_id:num(h.id), order_id:num(h.order_id), handle_id:num(h.handle_id) });
  ...
  for (const b of orphanBagHistory) push('error', 'سجل مخزن شنط مرتبط بأوردر محذوف', `حركة شنط #${num(b.id)} مرتبطة بأوردر غير موجود #${num(b.order_id)}`, { history_id:num(b.id), order_id:num(b.order_id), bag_id:num(b.bag_id) });
...
app.get('/system-health-check', authRequired, requirePerm('perm_system_config'), async (req, res) => {
  try { res.json(await runSystemHealthCheck()); } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/system-resync-all', authRequired, requirePerm('perm_system_config'), async (req, res) => {
  try {
    ...
    res.json({ success:true, result, health: result.final_health || await runSystemHealthCheck() });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.delete('/order-file/:id/delete', authRequired, requirePerm('perm_edit_order'), async (req, res) => {
  try {
    ...
    const keyName = String(file.filename || '').trim();
    let rows = [];
    if (keyPath && keyName) rows = await allAsync(`SELECT * FROM order_files WHERE filepath=? OR filename=?`, [keyPath, keyName]);
    else if (keyPath) rows = await allAsync(`SELECT * FROM order_files WHERE filepath=?`, [keyPath]);
    else if (keyName) rows = await allAsync(`SELECT * FROM order_files WHERE filename=?`, [keyName]);
...
app.post('/update-order', authRequired, requirePerm('perm_edit_order'), upload.array('designFiles'), async (req, res) => {
  try {
    ...
    const total = preserveGroupFinancials ? num(storedGroupFinance?.total) : num(b.total_price);
    let paid = preserveGroupFinancials ? Math.min(num(storedGroupFinance?.paid), total) : Math.min(num(b.paid_amount), total);
    ...
    const remaining = Math.max(total - paid, 0);
    const paymentType = remaining <= 0 ? 'مدفوع كامل' : (requestedPaymentType || before.paymentType || 'لم يتم الدفع');
    ...
    const nextNotes = Object.prototype.hasOwnProperty.call(b, 'notes') ? String(b.notes || '').trim() : String(before.notes || '').trim();
    ...
    try {
      ...
      const orderDiff = describeDiff(before, { custName: b.custName || '', custPhone: nextCustPhone, custAddress: b.custAddress || '', l: num(b.l), w: num(b.w), g: num(b.g), qty: num(b.qty), color: b.color || '', handle: b.handle || '', printType: nextPrintType, colorSpecs: b.colorSpecs || '', total_price: total, paid_amount: paid, remaining_amount: remaining, paymentType, status: nextStatus, paperGrammage: num(b.paperGrammage), useReadyStock: num(b.useReadyStock), priority, due_date: dueDate || null, shipping_cost: shippingCost, notes: nextNotes, order_page_name: nextOrderPageName });
      await recordAudit({
        req,
        action: 'update-order',
        entity_type: 'order',
...
app.post(BOSTA_WEBHOOK_PATH, async (req, res) => {
  try {
    ...
    for (const order of orders) {
      ...
      if (String(refreshed?.status || '').trim() === 'تم التسليم') await syncDeliveredShipmentPaymentState(refreshed, 'Bosta Webhook', true);
...
app.delete('/delete-order/:id', authRequired, requirePerm('perm_delete_order'), async (req, res) => {
  try {
    ...
    const savedCutAllocationsForDelete = parseSavedCutAllocations(order);
    const cutPaperIdsForDelete = uniqueList(savedCutAllocationsForDelete.map(row => row.paperId).concat(num(order.last_cut_paper_id))).filter(Boolean);
    const undoPayload = [
      ...
      ...(num(order.paper_cut_done) && cutPaperIdsForDelete.length ? await buildUndoPayloadFromDefs(paperSnapshotDefs(cutPaperIdsForDelete)) : []),
      ...(bagCouldChange ? [
        await snapshotTableSubset('bags', bagCriteria),
        await snapshotTableSubset('bags_history', criteriaEq('order_id', id))
      ].filter(Boolean) : []),
    ...
    try {
      ...
      if (num(order.paper_cut_done) && savedCutAllocationsForDelete.length) {
        for (const allocation of savedCutAllocationsForDelete) {
          const paper = await getAsync(`SELECT * FROM paper WHERE id=?`, [allocation.paperId]);
          if (!paper) continue;
          const restoreSheets = allocation.sheetsToUse;
          const restoreKg = allocation.kgToUse > 0 ? allocation.kgToUse : +(paperSheetWeightKg(paper) * restoreSheets).toFixed(6);
          await runAsync(`UPDATE paper SET total_kg=COALESCE(total_kg,0)+?, total_sheets=COALESCE(total_sheets,0)+? WHERE id=?`, [restoreKg, restoreSheets, paper.id]);
          await runAsync(`INSERT INTO paper_history (paper_id,order_id,type,kg,sheets,color,length,width,grammage,paper_name,note) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [paper.id, order.id, 'add', restoreKg, restoreSheets, paper.color, paper.length, paper.width, paper.grammage, paper.paper_name || '', `استرجاع ورق خام بسبب حذف ${await orderDisplayLabelForId(order.id, order.custName || '')}`]);
        }
      } else if (num(order.paper_cut_done) && num(order.last_cut_paper_id)) {
        const paper = await getAsync(`SELECT * FROM paper WHERE id=?`, [num(order.last_cut_paper_id)]);
        if (paper) {
          let sheets = 0, kg = 0;
          ...
          if (kg > 0 || sheets > 0) {
            ...
            await runAsync(`INSERT INTO paper_history (paper_id,order_id,type,kg,sheets,color,length,width,grammage,paper_name,note) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [paper.id, order.id, 'add', kg, sheets, paper.color, paper.length, paper.width, paper.grammage, paper.paper_name || '', `استرجاع ورق خام بسبب حذف الأوردر #${order.id}`]);
          }
        }
      }
    ...
    await recordAudit({
      ...
      touch_refs: [
        ...
        ...paperTouchRefs(cutPaperIdsForDelete),
        ...(bagAfter ? bagTouchRefs([bagAfter.id]) : []),
        ...(handleAfter ? handleTouchRefs([handleAfter.id]) : [])
      ]
    });
...
app.get('/cuttable-orders', authRequired, requirePerm('perm_cut_paper'), async (req, res) => {
  try {
    ...
    const rows = await allAsync(`SELECT id,custName,status,orderDate,l,w,g,qty,color,handle,printType,useReadyStock,paper_cut_done FROM orders WHERE TRIM(COALESCE(status,'')) IN ('أوردر جديد','تحت الإنتاج','مستني الزنكات') AND COALESCE(useReadyStock,0)=0 ORDER BY id DESC`);
    for (const row of rows) await attachOrderDisplayFields(row, row.id, row.custName || '');
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.get('/order-paper-plan/:id', authRequired, requireAnyPerm('perm_view_orders','perm_calculator','perm_cut_paper'), async (req, res) => {
  try {
    await ensureAllPaperDerivedBalances();
    const order = await getAsync(`SELECT * FROM orders WHERE id=?`, [num(req.params.id)]);
    if (!order) return res.status(404).json({ error: 'الأوردر غير موجود' });
    await attachOrderDisplayFields(order, order.id, order.custName || '');
    const wantedType = normalizePaperType(order.paper_type);
    const wantedColor = forcePaperColorForType(order.color, wantedType);
    const wantedGram = num(order.paperGrammage);
    const papers = (await allAsync(`SELECT * FROM paper WHERE total_sheets > 0 OR total_kg > 0 ORDER BY id DESC`))
      .filter(p => paperTypeFromRow(p) === wantedType && paperMatchesOrderTypeAndColor(order, p));
    let allOptions = [];
    ...
    const sortOptions = (rows=[]) => rows.slice().sort((x, y) => {
      if (x.wastePercent !== y.wastePercent) return x.wastePercent - y.wastePercent;
      return x.neededSheets - y.neededSheets;
    });
    const sufficientOptions = allOptions.filter(o => !!o.enoughStock);
    const displayOptions = sortOptions(sufficientOptions
      .filter(o => wantedGram <= 0 || Math.abs(num(o.paperGrammage) - wantedGram) < 0.001)
      .map(o => ({ ...o, isAlternative: false })));
    const alternativeOptions = wantedGram > 0
      ? sortOptions(sufficientOptions
          .filter(o => Math.abs(num(o.paperGrammage) - wantedGram) >= 0.001)
          .map(o => ({ ...o, isAlternative: true })))
      : [];
    const shortageOptions = allOptions.filter(o => !o.enoughStock && (wantedGram <= 0 || Math.abs(num(o.paperGrammage) - wantedGram) < 0.001));
    const splitPlan = chooseCombinedCutPlan(order, papers, { wantedGram, allowDifferentGram: false });
    const alternativeSplitPlan = wantedGram > 0
      ? chooseCombinedCutPlan(order, papers, { wantedGram, allowDifferentGram: true })
      : null;
    const splitPlanOptions = listCombinedCutPlanOptions(order, papers, { wantedGram });
    const splitPaperChoices = listSplitPaperChoices(order, papers, { wantedGram });

    let actualCut = null;
    let actualCutAllocations = [];
    if (num(order.paper_cut_done) && String(order.last_cut_allocations || '').trim()) {
      let savedAllocations = [];
      try { savedAllocations = JSON.parse(String(order.last_cut_allocations || '[]')); } catch (_) { savedAllocations = []; }
      for (const saved of (Array.isArray(savedAllocations) ? savedAllocations : [])) {
        const savedPaper = await getAsync(`SELECT * FROM paper WHERE id=?`, [num(saved?.paperId ?? saved?.paper_id)]);
        const savedLayout = String(saved?.layoutKey || order.last_cut_layout || 'pieceByPiece').trim() === 'singlePiece' ? 'singlePiece' : 'pieceByPiece';
        if (!savedPaper) continue;
        const computed = computeLayout(order, savedPaper, savedLayout);
        actualCutAllocations.push({
          ...(computed || {}),
          ...saved,
          paperId: savedPaper.id,
          paperLabel: paperLabelBase(savedPaper),
          paperLabelFull: paperLabelFull(savedPaper),
          paperColor: savedPaper.color,
          paperGrammage: num(savedPaper.grammage),
          layoutKey: savedLayout,
          layoutLabel: savedLayout === 'singlePiece' ? 'حته واحدة' : 'حته ف حته',
          neededSheets: Math.max(0, Math.round(num(saved?.sheetsToUse ?? saved?.neededSheets))),
          sheetsToUse: Math.max(0, Math.round(num(saved?.sheetsToUse ?? saved?.neededSheets))),
          isActualCut: true,
          enoughStock: true
        });
      }
      actualCut = actualCutAllocations[0] || null;
    }
    if (!actualCut && num(order.paper_cut_done) && num(order.last_cut_paper_id)) {
      const actualPaper = await getAsync(`SELECT * FROM paper WHERE id=?`, [num(order.last_cut_paper_id)]);
      const layoutKey = String(order.last_cut_layout || 'pieceByPiece').trim() || 'pieceByPiece';
      if (actualPaper) {
    ...
    res.json({
      ...
      alternativeOptions,
      splitPlan,
      alternativeSplitPlan: alternativeSplitPlan?.usesAlternativeGram ? alternativeSplitPlan : null,
      splitPlanOptions,
      splitPaperChoices,
      requiredPaper: { paperType: wantedType, color: wantedColor, grammage: wantedGram },
      hasRequiredStockShortage: shortageOptions.length > 0,
      requiredStockShortageCount: shortageOptions.length,
      actualCut,
      actualCutAllocations
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/order-paper-plan/:id/custom-split', authRequired, requireAnyPerm('perm_view_orders','perm_calculator','perm_cut_paper'), async (req, res) => {
  try {
    if (!canManageInventory(req.user)) return res.status(403).json({ error: 'غير مصرح' });
    await ensureAllPaperDerivedBalances();
    const order = await getAsync(`SELECT * FROM orders WHERE id=?`, [num(req.params.id)]);
    if (!order) return res.status(404).json({ error: 'الأوردر غير موجود' });
    const paperIds = uniqueList(Array.isArray(req.body?.paper_ids) ? req.body.paper_ids.map(num) : []).filter(Boolean).slice(0, 3);
    if (paperIds.length < 2) return res.status(400).json({ error: 'اختار الصنف الثابت وصنف استكمال واحد على الأقل' });
    const placeholders = paperIds.map(() => '?').join(',');
    const selectedRows = await allAsync(`SELECT * FROM paper WHERE id IN (${placeholders})`, paperIds);
    const selectedMap = new Map(selectedRows.map(row => [num(row.id), row]));
    const selectedPapers = paperIds.map(id => selectedMap.get(num(id))).filter(Boolean);
    if (selectedPapers.length !== paperIds.length) return res.status(404).json({ error: 'أحد أصناف الورق المختارة لم يعد موجودًا' });
    for (const paper of selectedPapers) {
      if (!paperMatchesOrderTypeAndColor(order, paper)) return res.status(400).json({ error: `الصنف ${paperLabelBase(paper)} ليس من نفس نوع ولون ورق الأوردر` });
      if (Math.floor(num(paper.total_sheets)) <= 0) return res.status(400).json({ error: `لا يوجد رصيد أفرخ متاح في ${paperLabelBase(paper)}` });
    }
    const wantedGram = num(order.paperGrammage);
    const plan = buildManualCombinedCutPlan(order, selectedPapers, { wantedGram });
    if (!plan) {
      const choices = listSplitPaperChoices(order, selectedPapers, { wantedGram });
      const combinedCapacity = choices.reduce((sum, row) => sum + num(row.capacityQty), 0);
      return res.status(400).json({ error: `الأصناف المختارة معًا لا تكفي كمية الأوردر بالطريقة المتاحة. أقصى تغطية تقريبية ${Math.round(combinedCapacity)} شنطة من أصل ${Math.round(num(order.qty))}. اختار صنف استكمال آخر.` });
    }
    res.json({ success: true, plan: { ...plan, manualSelection: 1 }, selectedPaperIds: paperIds });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/production-plan', authRequired, requirePerm('perm_manage_bags'), async (req, res) => {
  try {
    ...
    if (normalizeReadyBagHandle(b.handle) === 'بيد' && isReadyBagNoHandleOnlySize(b.l, b.w)) return res.status(400).json({ error: 'مقاس 10×10 و10×15 لا يركب له يد نهائيًا؛ اختار بدون يد' });
...
app.post('/create-handle-install-production-order', authRequired, requirePerm('perm_manage_bags'), async (req, res) => {
  try {
    ...
    if (isReadyBagNoHandleOnlySize(sourceBag.length, sourceBag.width)) return res.status(400).json({ error: 'مقاس 10×10 و10×15 لا يركب له يد نهائيًا' });
    ...
    const availableHandle = await getAsync(`SELECT * FROM handles WHERE color=? AND COALESCE(handle_type,'مجدول')=? ORDER BY id DESC LIMIT 1`, [String(sourceBag.color || '').trim(), handleType])
      || await getAsync(`SELECT * FROM handles WHERE color=? ORDER BY id DESC LIMIT 1`, [String(sourceBag.color || '').trim()]);
    if (!availableHandle) return res.status(400).json({ error: 'لا يوجد يد مطابقة لهذا اللون في مخزن اليد' });
    if (num(availableHandle.qty) < qty) return res.status(400).json({ error: `كمية اليد غير كافية في المخزن. المطلوب ${qty} والمتاح ${num(availableHandle.qty)}` });
    const manualCostHandFix = String(b.cost_hand_fix ?? '').trim() === '' ? roundMoney(qty * READY_BAG_HANDLE_FIX_UNIT_COST) : roundMoney(Math.max(0, num(b.cost_hand_fix)));
...
app.post('/create-production-order', authRequired, requirePerm('perm_manage_bags'), async (req, res) => {
  try {
    ...
    if (handle === 'بيد' && isReadyBagNoHandleOnlySize(l, w)) return res.status(400).json({ error: 'مقاس 10×10 و10×15 لا يركب له يد نهائيًا؛ اختار بدون يد' });
...
// بيانات الورق فقط لمخطط القص؛ الصلاحية مستقلة عن الحاسبة والمخزن.
app.get('/cutting-optimizer-stock-data', authRequired, requirePerm('perm_cutting_optimizer'), async (req, res) => {
  try {
    const paper = (await allAsync(`SELECT * FROM paper WHERE COALESCE(total_sheets,0)>0 ORDER BY id DESC`))
      .map(row => paperWithPricingVisibility(row, req.user));
    // أصناف الشنط هنا ككتالوج مقاسات للحشو داخل هدر الفرخ فقط؛ لا يتم خصم أي شنط جاهزة.
    const bags = await allAsync(`SELECT id,length,width,gusset,color,handle,total_qty FROM bags WHERE COALESCE(length,0)>0 AND COALESCE(width,0)>0 ORDER BY id DESC`);
    const handles = await allAsync(`SELECT id,color,COALESCE(handle_type,'مجدول') AS handle_type,qty FROM handles WHERE COALESCE(qty,0)>0 ORDER BY id DESC`);
    res.json({ paper, bags, handles });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/cutting-optimizer-production-orders', authRequired, requirePerm('perm_cut_paper'), async (req, res) => {
  ...
  try {
    ...
    const costCut = String(body.cost_cut ?? '').trim() === '' ? READY_BAG_CUT_FLAT_COST : roundMoney(Math.max(0, num(body.cost_cut)));
    const costMake = totalQty > 0 ? (String(body.cost_make ?? '').trim() === '' ? roundMoney(totalQty * READY_BAG_MAKE_UNIT_COST) : roundMoney(Math.max(0, num(body.cost_make)))) : 0;
    const costHandFix = handledQty > 0 ? (String(body.cost_hand_fix ?? '').trim() === '' ? roundMoney(handledQty * READY_BAG_HANDLE_FIX_UNIT_COST) : roundMoney(Math.max(0, num(body.cost_hand_fix)))) : 0;
    ...
    try {
      ...
      for (let index = 0; index < productionItems.length; index += 1) {
        ...
        const itemHandCost = item.handle === 'بيد' ? roundMoney(item.qty * READY_BAG_HANDLE_UNIT_COST) : 0;
      ...
      if (!productionItems.length && operationExpense > 0) {
        ...
        const expenseInsert = await runAsync(`INSERT INTO expenses (expense_date,amount,category,custom_category,notes,linked_to_order,order_id,order_cost_field,expense_partner_name,actor_username,actor_name,source_type,source_ref,is_auto,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [today(), operationExpense, 'تكاليف قص ورق', '', expenseNote, 0, null, '', '', normalizeActorUsername(actorInfo.username), actorInfo.full_name, 'cutting_optimizer_cut_stock', sourceRef, 1, creator]);
...
// Paper
app.get('/get-paper', authRequired, requirePerm('perm_view_inventory'), async (req, res) => { try { await ensureAllPaperDerivedBalances(); const rows=(await allAsync(`SELECT * FROM paper ORDER BY id DESC`)).map(row => paperWithPricingVisibility(row, req.user)); res.json(rows); } catch (e) { res.status(500).json({ error: e.message }); } });
app.post('/add-paper', authRequired, requirePerm('perm_manage_paper'), async (req, res) => {
  try {
    ...
    if (movementPaperId) {
      ...
      const updateResult = await runAsync(`UPDATE paper SET total_kg=MAX(0,COALESCE(total_kg,0)+?), total_sheets=MAX(0,COALESCE(total_sheets,0)+?), min_kg=?, min_sheets=?, buy_price_kg=?, buy_price_sheet=? WHERE id=?`, [kg, sheets, b.min_kg === undefined || b.min_kg === '' ? num(row.min_kg) : num(b.min_kg), b.min_sheets === undefined || b.min_sheets === '' ? num(row.min_sheets) : num(b.min_sheets), nextBuyPriceKg, nextBuyPriceSheet, row.id]);
      ...
      await recordAudit({ req, action: 'paper-stock-movement', entity_type: 'paper', entity_id: row.id, details: `${kind === 'sub' ? 'خصم' : 'إضافة'} ورق ${paper.color || ''} ${paper.length}×${paper.width} - ${paper.grammage} جم${paper.paper_name ? ' - ' + paper.paper_name : ''} | ${kg.toFixed(2)} كجم | ${Math.round(sheets)} فرخ`, can_undo: 1, undo_type: 'paper-stock-movement', undo_payload: undoPayload, touch_refs: paperTouchRefs([row.id]) });
    ...
    if (editId) {
      ...
      await runAsync(`UPDATE paper SET length=?,width=?,grammage=?,color=?,paper_name=?,paper_type=?,total_kg=?,total_sheets=?,min_kg=?,min_sheets=?,buy_price_kg=?,buy_price_sheet=? WHERE id=?`, [num(b.length), num(b.width), num(b.grammage), b.color || '', paperName, paperType, editMovement.kg, editMovement.sheets, num(b.min_kg), num(b.min_sheets), buyPriceKg, buyPriceSheet, editId]);
...
app.post('/cut-paper', authRequired, requirePerm('perm_cut_paper'), async (req, res) => {
  try {
    ...
    const singleAllocation = JSON.stringify([{ paperId, paperLabel:paperLabelBase(paper), paperLabelFull:paperLabelFull(paper), paperColor:paper.color, paperGrammage:num(paper.grammage), layoutKey:layoutKey || 'pieceByPiece', layoutLabel:(layoutKey || 'pieceByPiece') === 'singlePiece' ? 'حته واحدة' : 'حته ف حته', sheetsToUse:sheets, neededSheets:sheets, kgToUse:kg, costToUse:+cost.toFixed(2), isAlternative:usesAlternativeGram }]);
...
app.get('/accounts-summary', authRequired, requirePerm('perm_view_accounts'), async (req, res) => {
  try {
    ...
    if (!canSeeFullAccounts(req.user)) {
      ...
      return res.json({ totalSales: 0, totalCosts: 0, totalExpenses: num(exp.totalExpenses), totalProfit: 0, actualProfit: 0, expectedProfit: 0, totalProfitWithExpected: 0, expectedProfitOrdersCount: 0, expectedProfitItemsCount: 0, expectedProfitSales: 0, expectedProfitCosts: 0, totalRemaining: 0, totalPurchases: 0, totalPurchasesRemaining: 0 });
    }
    const from = String(req.query.from || '').trim();
    const to = String(req.query.to || '').trim();
    ...
    const actualProfit = roundMoney(num(sales.totalProfit) - num(exp.totalExpenses));
...
app.get('/outflow-items', authRequired, requirePerm('perm_view_accounts'), async (req, res) => {
  try {
    ...
    // تفاصيل إجمالي الخارج لازم تعكس الفلوس التي خرجت فعليًا فقط.
    // سجل التكاليف يعرض كل التكاليف المستحقة على الأوردرات، لكن إجمالي الخارج/السيولة يعرض المصروفات المدفوعة فقط.
    const orderCostFilter = buildDateFilterParts('e.expense_date', from, to);
    const paidOrderCosts = await allAsync(`SELECT e.*, o.custName FROM expenses e LEFT JOIN orders o ON o.id=e.order_id WHERE COALESCE(e.linked_to_order,0)=1${orderCostFilter.sql} ORDER BY e.expense_date DESC, e.id DESC`, orderCostFilter.params);
    await enrichRowsWithOrderDisplay(paidOrderCosts, 'order_id');
    for (const e of paidOrderCosts) {
      const field = String(e.order_cost_field || '').trim();
      rows.push({
        key: `order_cost:${num(e.order_id)}:${field}`,
        type: 'order_cost',
        id: num(e.id),
        order_id: num(e.order_id),
        field,
        source_ref: String(e.source_ref || '').trim(),
        date: e.expense_date || '',
        source: `${costFieldLabel(field)} - ${e.order_display_label || ('أوردر #' + num(e.order_id))}`,
        details: e.notes || '',
        amount: num(e.amount),
        actor_username: normalizeActorUsername(e.actor_username),
        actor_name: e.actor_name || '',
        assignable: num(e.amount) > 0 ? 1 : 0
      });
    }
    const productionFilter = buildDateFilterParts('e.expense_date', from, to);
    const paidProductionCosts = await allAsync(`SELECT e.* FROM expenses e WHERE COALESCE(e.source_type,'')='production_order'${productionFilter.sql} ORDER BY e.expense_date DESC, e.id DESC`, productionFilter.params);
    for (const e of paidProductionCosts) {
      rows.push({
        key: `production:${String(e.source_ref || '').trim()}`,
        type: 'production',
        id: num(e.id),
        order_id: 0,
        ...
        source_ref: String(e.source_ref || '').trim(),
        date: e.expense_date || '',
        source: `أمر تشغيل شنط جاهزة #${String(e.source_ref || '').trim()}`,
        details: e.notes || '',
        amount: num(e.amount),
        actor_username: normalizeActorUsername(e.actor_username),
        actor_name: e.actor_name || '',
...
app.get('/dashboard-data', authRequired, async (req, res) => {
  try {
    ...
    // أوامر تصنيع المخزون تخفض السيولة وتزيد تكلفة المخزون، ولا تخفض الربح قبل البيع.
    res.json({ totalSales: num(sales.totalSales), totalCosts: num(costs.totalCosts) + num(productionCosts?.totalCosts), totalExpenses: num(exp.totalExpenses), totalProfit: num(sales.bad) - num(exp.totalExpenses), totalRemaining: num(sales.totalRemaining), paperSheetsTotal: num(paper.paperSheetsTotal), bagsQtyTotal: num(bags.bagsQtyTotal), handlesQtyTotal: num(handles.handlesQtyTotal) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/save-purchase', authRequired, requirePerm('perm_purchases'), async (req, res) => {
  try {
    ...
    if (paidAmount > 0) {
      const payment = await runAsync(`INSERT INTO purchase_payments (purchase_id,supplier_id,amount,payment_date,note,created_by) VALUES (?,?,?,?,?,?)`, [purchase.id, supplierId, paidAmount, purchaseDate, 'دفعة أولية مع فاتورة الشراء', req.user.full_name || req.user.username]);
      await syncPurchasePaymentAdminCash(payment.lastID, req.user.full_name || req.user.username);
    }
...
app.post('/add-purchase-payment/:id', authRequired, requirePerm('perm_purchases'), async (req, res) => {
  try {
    ...
    const payment = await runAsync(`INSERT INTO purchase_payments (purchase_id,supplier_id,amount,payment_date,note,created_by) VALUES (?,?,?,?,?,?)`, [purchaseId, num(purchase.supplier_id), amount, String(req.body?.payment_date || today()).trim() || today(), String(req.body?.note || '').trim(), req.user.full_name || req.user.username]);
    await syncPurchasePaymentAdminCash(payment.lastID, req.user.full_name || req.user.username);
...
app.delete('/purchase-payment/:id', authRequired, requirePerm('perm_purchases'), async (req, res) => {
  try {
    ...
    await deleteAdminCashEntriesBySource('purchase_payment', String(num(payment.id)));
...
app.get('/reports-dashboard', authRequired, requirePerm('perm_view_reports'), async (req, res) => {
  try {
    await syncAllSalesHistory();
    const [sales, purchases, expenses, receivables, payables, delivered, pending, lateOrders, topCustomers, topSizes, monthlySales, printedReturnedLosses, lowPaper, lowBags, lowHandles, topSuppliers] = await Promise.all([
      ...
      getAsync(`SELECT COUNT(*) c FROM orders WHERE TRIM(COALESCE(status,'')) IN ('أوردر جديد','تحت الإنتاج','مستني الزنكات','في القص','تحت الطباعة','تحت التصنيع','جاهز للشحن','تم الشحن')`),
      ...
      allAsync(`SELECT o.id,o.custName,o.status,o.printType,o.total_price,o.group_code,o.item_no,o.item_count,s.shipping_cost,s.total_cost,s.net_profit,s.sale_date FROM sales_history s JOIN orders o ON o.id=s.order_id WHERE TRIM(COALESCE(o.status,''))='مرتجع' AND TRIM(COALESCE(o.printType,'')) IN ('سلك سكرين','أوفست') ORDER BY COALESCE(s.sale_date,'') DESC, o.id DESC LIMIT 100`),
    ...
    res.json({
      ...
      printedReturnedLosses: await enrichRowsWithOrderDisplay(printedReturnedLosses || [], 'id'),
...
app.get('/order-operations/:id', authRequired, requireAnyPerm('perm_view_artisans','perm_edit_order'), async (req, res) => {
  try {
...
app.post('/save-order-operation', authRequired, requirePerm('perm_view_artisans'), requirePerm('perm_edit_order'), async (req, res) => {
  try {
    ...
    const linkedField = executionCostField(stepType);
    const operationUndoPayload = await buildUndoPayloadFromDefs([
      ...orderSnapshotDefs([orderId]),
      ...(linkedField ? [{ table:'expenses', criteria: criteriaEq('source_ref', `order:${orderId}:${linkedField}`) }] : [])
    ]);
    ...
    if (num(b.id) > 0) {
      ...
      await recordAudit({ req, action: 'update-order-operation', entity_type: 'order_operation', entity_id: operationId, details: `تعديل مرحلة ${stepLabel} - ${orderLabelForUi} | ${partnerName || partnerType || '-'} | قيمة ${amount.toFixed(2)} ج${paymentRequested ? ` | تم الدفع من ${actorInfo.full_name || actorInfo.username}` : ' | تكلفة فقط بدون خصم كاش'}`, can_undo: 1, undo_type: 'update-order-operation', undo_payload: operationUndoPayload, touch_refs: orderTouchRefs([orderId]) });
    ...
    await syncOrderOperationalStatus(orderId, req.user.full_name || req.user.username, req);
    await recordAudit({ req, action: 'create-order-operation', entity_type: 'order_operation', entity_id: ins.lastID, details: `إضافة مرحلة ${stepLabel} - ${orderLabelForUi} | ${partnerName || partnerType || '-'} | كمية ${Math.max(0, Math.round(num(b.qty) || num(order.qty)))} | قيمة ${amount.toFixed(2)} ج${paymentRequested ? ` | تم الدفع من ${actorInfo.full_name || actorInfo.username}` : ' | تكلفة فقط بدون خصم كاش'}`, can_undo: 1, undo_type: 'create-order-operation', undo_payload: operationUndoPayload, touch_refs: orderTouchRefs([orderId]) });
...
app.delete('/delete-order-operation/:id', authRequired, requirePerm('perm_view_artisans'), requirePerm('perm_edit_order'), async (req, res) => {
  try {
    ...
    const linkedField = row?.order_id ? executionCostField(row.step_type) : '';
    const undoPayload = row?.order_id ? await buildUndoPayloadFromDefs([
      ...orderSnapshotDefs([row.order_id]),
      ...(linkedField ? [{ table:'expenses', criteria: criteriaEq('source_ref', `order:${num(row.order_id)}:${linkedField}`) }] : [])
    ]) : [];
    ...
    await recordAudit({ req, action: 'delete-order-operation', entity_type: 'order_operation', entity_id: id, details: row ? `حذف مرحلة ${row.step_label || row.step_type} من ${await orderDisplayLabelForId(row.order_id)}${linkedPayments.length ? ' | تم حذف الدفعة المرتبطة تلقائيًا' : ''}` : 'حذف مرحلة تشغيل', can_undo: row ? 1 : 0, undo_type: 'delete-order-operation', undo_payload: undoPayload, touch_refs: row?.order_id ? orderTouchRefs([row.order_id]) : [] });
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.get('/operations-partners-summary', authRequired, requirePerm('perm_view_artisans'), async (req, res) => {
  try {
    // حساب الجهة هنا كشف حساب فقط: الشغل = مستحق له، والدفعات = تسويات مدخلة يدويًا.
    // خصم تكلفة الأوردر من الخزنة يتم تلقائيًا في الحسابات ولا يعتبر دفعة للصنايعي حتى لا يصفّر رصيده.
    ...
    const out = [];
    for (const partner of partners) {
      const jobs = await allAsync(`SELECT oo.*, o.custName, o.status AS order_status, ${operationAccountDeductedSql('oo')} AS account_deducted_amount FROM order_operations oo LEFT JOIN orders o ON o.id=oo.order_id WHERE oo.partner_id=? AND COALESCE(o.useReadyStock,0)=0 AND TRIM(COALESCE(o.status,'')) NOT IN ('تم الشحن','تم التسليم','مرتجع') ORDER BY oo.id DESC`, [partner.id]);
      await enrichRowsWithOrderDisplay(jobs, 'order_id');
      const manualPay = await getAsync(`SELECT COALESCE(SUM(amount),0) total_paid FROM partner_payments WHERE partner_id=? AND COALESCE(auto_created,0)=0`, [partner.id]);
      const totalAssigned = roundMoney(jobs.reduce((s, r) => s + num(r.amount), 0));
      const manualPaid = roundMoney(num(manualPay?.total_paid));
      const cashDeducted = roundMoney(jobs.reduce((s, r) => s + Math.min(num(r.amount), num(r.account_deducted_amount)), 0));
      out.push({
        ...partner,
        jobs_count: jobs.length,
        done_jobs: jobs.filter(r => String(r.status || '').trim() === 'done').length,
        total_assigned: +totalAssigned.toFixed(2),
        total_paid: +manualPaid.toFixed(2),
        auto_paid: +cashDeducted.toFixed(2),
        cash_deducted: +cashDeducted.toFixed(2),
        manual_paid: +manualPaid.toFixed(2),
        remaining: +(Math.max(totalAssigned - manualPaid, 0)).toFixed(2),
        jobs: jobs.map(j => ({ ...j, financially_settled: 0, cash_deducted: num(j.account_deducted_amount) >= num(j.amount) && num(j.amount) > 0 ? 1 : 0 }))
      });
    }
    res.json(out);
  } catch (e) { res.status(500).json({ error: e.message }); }
...
app.get('/partner-payments/:id', authRequired, requirePerm('perm_view_artisans'), async (req, res) => {
  try {
    ...
    const rows = await allAsync(`SELECT id,partner_id,partner_name,amount,payment_date,note,created_by,payment_scope,0 AS from_expense FROM partner_payments WHERE partner_id=? AND COALESCE(auto_created,0)=0 ORDER BY payment_date DESC,id DESC`, [partnerId]);
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/save-partner-payment', authRequired, requirePerm('perm_view_artisans'), requirePerm('perm_edit_order'), async (req, res) => {
  try {
    ...
    const ins = await runAsync(`INSERT INTO partner_payments (partner_id,partner_name,amount,payment_date,note,created_by,payment_scope,auto_created) VALUES (?,?,?,?,?,?,?,0)`, [partner.id, partner.name, amount, paymentDate, paymentNote || 'تسوية حساب مجمعة بدون خصم من الخزنة', req.user.full_name || req.user.username, 'settlement']);
    await recordAudit({ req, action: 'partner-payment', entity_type: 'partner_payment', entity_id: ins.lastID, details: `تسجيل دفعة/تسوية ${amount.toFixed(2)} ج إلى ${partner.name} | بدون خصم من الخزنة لأن تكلفة الأوردرات مخصومة تلقائيًا` });
    res.json({ success: true, id: ins.lastID, cash_deducted: false });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/pay-order-operations', authRequired, requirePerm('perm_view_artisans'), requirePerm('perm_edit_order'), async (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      const partnerId = num(row.partner_id);
      if (!partnerId || amount <= 0) continue;
      if (!byPartner.has(partnerId)) byPartner.set(partnerId, { partnerId, partnerName: String(row.partner_name || '').trim(), total: 0, count: 0 });
      const item = byPartner.get(partnerId);
      item.total = roundMoney(item.total + amount);
      item.count++;
    }
    let total = 0, paidCount = 0;
    for (const item of byPartner.values()) {
      const partner = await getAsync(`SELECT * FROM execution_partners WHERE id=?`, [item.partnerId]);
      const partnerName = String(partner?.name || item.partnerName || '').trim();
      const ins = await runAsync(`INSERT INTO partner_payments (partner_id,partner_name,amount,payment_date,note,created_by,payment_scope,auto_created) VALUES (?,?,?,?,?,?,?,0)`, [item.partnerId, partnerName, item.total, paymentDate, note || `تسوية مجمعة لعدد ${item.count} تكليف بدون خصم من الخزنة`, req.user.full_name || req.user.username, 'settlement']);
      await recordAudit({ req, action: 'partner-payment', entity_type: 'partner_payment', entity_id: ins.lastID, details: `تسوية مجمعة ${item.total.toFixed(2)} ج إلى ${partnerName} | بدون خصم من الخزنة` });
      total = roundMoney(total + item.total);
      paidCount += item.count;
    }
    res.json({ success: true, count: paidCount, total, cash_deducted: false });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/operations-dashboard', authRequired, requirePerm('perm_view_artisans'), async (req, res) => {
  try {
    ...
    const paid = await getAsync(`SELECT COALESCE(SUM(amount),0) paid FROM partner_payments WHERE COALESCE(auto_created,0)=0`);
    const deducted = await getAsync(`SELECT COALESCE(SUM(e.amount),0) deducted FROM expenses e WHERE COALESCE(e.linked_to_order,0)=1 AND COALESCE(e.execution_partner_id,0)>0`);
    res.json({ active_orders: num(activeOrders?.c), pending_operations: num(pendingOps?.c), active_partners: num(partners?.c), total_assigned: +num(finances?.assigned).toFixed(2), total_paid: +num(paid?.paid).toFixed(2), total_cash_deducted: +num(deducted?.deducted).toFixed(2), remaining: +(Math.max(num(finances?.assigned) - num(paid?.paid), 0)).toFixed(2) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/backup-db', authRequired, requirePerm('perm_backup_restore'), async (req, res) => {
  try {
    ...
    res.json({ success: true, url: createProtectedDownloadUrl('backups', name, req.user), fileName: name, mode: 'full-system-backup' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/backup-db-file', authRequired, requirePerm('perm_backup_restore'), async (req, res) => {
  try {
    ...
    await createConsistentDatabaseSnapshot(dbCopyPath);
    await recordAudit({ req, action: 'backup-db', entity_type: 'system', entity_id: null, details: `تصدير قاعدة البيانات: ${name}` });
    res.json({ success: true, url: createProtectedDownloadUrl('backups', name, req.user), fileName: name, mode: 'database-only-backup' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
function auditActionLabel(action='') {
  ...
  const map = {
    'update-status':'تغيير حالة أوردر',
    'auto-update-status':'تغيير حالة تلقائي',
    'cut-paper':'قص ورق لأوردر',
    'cut-paper-ready-bags':'قص ورق لأمر تشغيل',
    'cutting-optimizer-production-orders':'تنفيذ مخطط القص الذكي',
    'update-order':'تعديل أوردر',
    'save-order':'إضافة أوردر',
    'delete-order':'حذف أوردر',
    'save-expense':'إضافة مصروف / تكلفة',
    'update-expense':'تعديل مصروف / تكلفة',
    'delete-expense':'حذف مصروف',
    'create-order-operation':'إضافة مرحلة تنفيذ',
    'update-order-operation':'تعديل مرحلة تنفيذ',
    'delete-order-operation':'حذف مرحلة تنفيذ',
    'add-paper-stock':'زيادة رصيد ورق',
    'paper-stock-movement':'حركة رصيد ورق',
    'sync-inventory-flags':'مزامنة علامات المخازن',
    'create-paper':'إضافة صنف ورق',
    'update-paper':'تعديل ورق',
    'delete-paper':'حذف ورق',
    'cash-adjustment':'تسوية سيولة',
    'update-cash-adjustment':'تعديل تسوية سيولة',
    'delete-cash-adjustment':'حذف تسوية سيولة',
    'add-purchase-payment':'دفعة مشتريات',
    'delete-purchase-payment':'حذف دفعة مشتريات',
    'save-purchase':'إضافة مشتريات',
    'delete-purchase':'حذف مشتريات',
    'create-partner':'إضافة جهة تنفيذ',
    'update-partner':'تعديل جهة تنفيذ',
    'delete-partner':'حذف جهة تنفيذ',
    'session-location':'تحديث موقع الجهاز'
  };
  return map[a] || a || '-';
}
function auditPayloadRows(payload = [], tableName = '') {
  const wanted = String(tableName || '').trim();
  const out = [];
  for (const item of (Array.isArray(payload) ? payload : [])) {
    if (String(item?.table || '').trim() !== wanted) continue;
    for (const row of (Array.isArray(item.rows) ? item.rows : [])) out.push(row);
  }
  return out;
}
function firstPayloadRowById(payload = [], tableName = '', id = 0) {
  const cleanId = num(id);
  return auditPayloadRows(payload, tableName).find(r => num(r.id) === cleanId) || null;
}
async function auditOrderLabelFor(orderId = 0, fallbackName = '') {
  const cleanId = num(orderId);
  if (!cleanId) return '';
  const meta = await getOrderDisplayMeta(cleanId);
  const label = formatOrderDisplayLabel({ ...meta, custName: fallbackName || meta.custName });
  return label + (num(meta.display_no) && num(meta.display_no) !== cleanId ? ` (ID ${cleanId})` : '');
}
async function enrichAuditLogRow(row = {}) {
  const type = String(row.entity_type || '').trim();
  const id = num(row.entity_id);
  const payload = safeJsonParse(row.undo_payload, []);
  row.action_label = auditActionLabel(row.action);
  row.entity_display_label = type ? `${type}${id ? ' #' + id : ''}` : '-';
  row.details_display = String(row.details || '').trim() || '-';
  row.touch_refs_list = auditTouchRefsForRow(row);

  try {
    if (type === 'order' && id) {
      const order = await getAsync(`SELECT id,custName FROM orders WHERE id=?`, [id]) || firstPayloadRowById(payload, 'orders', id) || {};
      const label = await auditOrderLabelFor(id, order.custName || '');
      row.entity_display_label = label || row.entity_display_label;
      if (label && row.details_display && !row.details_display.includes(label)) row.details_display += ` | الرقم المعروض: ${label}`;
    } else if (type === 'order_operation' && id) {
      const op = await getAsync(`SELECT oo.*, o.custName FROM order_operations oo LEFT JOIN orders o ON o.id=oo.order_id WHERE oo.id=?`, [id]) || firstPayloadRowById(payload, 'order_operations', id) || {};
      const orderId = num(op.order_id);
      const orderLabel = orderId ? await auditOrderLabelFor(orderId, op.custName || '') : '';
      const step = String(op.step_label || op.step_type || '').trim();
      const partner = String(op.partner_name || '').trim();
      row.entity_display_label = [step || 'مرحلة تنفيذ', orderLabel, partner].filter(Boolean).join(' - ') || row.entity_display_label;
      if (orderLabel && !row.details_display.includes(orderLabel)) row.details_display += ` | الرقم المعروض: ${orderLabel}`;
    } else if (type === 'expense' && id) {
      const exp = await getAsync(`SELECT e.*, o.custName FROM expenses e LEFT JOIN orders o ON o.id=e.order_id WHERE e.id=?`, [id]) || firstPayloadRowById(payload, 'expenses', id) || {};
      const orderId = num(exp.order_id);
      if (orderId) {
        const orderLabel = await auditOrderLabelFor(orderId, exp.custName || '');
        row.entity_display_label = `مصروف/تكلفة #${id} - ${orderLabel}`;
        if (orderLabel && !row.details_display.includes(orderLabel)) row.details_display += ` | الرقم المعروض: ${orderLabel}`;
      } else {
        row.entity_display_label = `مصروف #${id}`;
      }
    } else if (type === 'paper' && id) {
      const paper = await getAsync(`SELECT * FROM paper WHERE id=?`, [id]) || firstPayloadRowById(payload, 'paper', id) || {};
      if (paper && (paper.length || paper.width || paper.grammage || paper.color || paper.paper_name)) {
        row.entity_display_label = `ورق #${id} - ${[paper.paper_name, paper.color, `${num(paper.length)}×${num(paper.width)}`, `${num(paper.grammage)} جم`].filter(v => String(v||'').trim() && String(v) !== '0×0' && String(v) !== '0 جم').join(' - ')}`;
      }
    } else if (type === 'purchase' && id) {
      const pur = await getAsync(`SELECT * FROM purchases WHERE id=?`, [id]) || firstPayloadRowById(payload, 'purchases', id) || {};
      row.entity_display_label = `مشتريات #${id}${pur.item_name ? ' - ' + String(pur.item_name).trim() : ''}`;
    } else if (type === 'partner' && id) {
      const partner = await getAsync(`SELECT * FROM execution_partners WHERE id=?`, [id]) || firstPayloadRowById(payload, 'execution_partners', id) || {};
      row.entity_display_label = `جهة تنفيذ #${id}${partner.name ? ' - ' + String(partner.name).trim() : ''}`;
    }
  } catch (_) {}
  return row;
}
...
app.get('/activity-logs', authRequired, requirePerm('perm_activity_logs'), async (req, res) => {
  try {
    ...
    for (const row of rows) await enrichAuditLogRow(row);
...
async function deleteAuditLogTargetCascade(log, req=null) {
  ...
  // حذف عملية من سجل الأدمن = إلغاء أثر العملية والرجوع للّقطة السابقة.
  // لا نمنع الحذف بسبب عمليات أحدث؛ endpoint بيحذف/يرجع العمليات الأحدث المرتبطة أولاً.
  ...
  // العمليات القديمة التي لم يكن لها snapshot لا يمكن إرجاعها بدقة.
  // نعمل fallback آمن فقط للعمليات التي أنشأت كيان واضح، ونتجنب حذف الأوردر بالكامل مع update-status أو update-order.
  const isCreateAction = /^(create|save|add)-/.test(action) || ['cash-adjustment','transfer-admin-cash'].includes(action);
  const isDeleteAction = /^delete-/.test(action);

  if (type === 'order' && id && (action === 'save-order' || action === 'create-order')) {
    ...
    await runAsync(`DELETE FROM partner_payments WHERE order_id=?`, [id]);
    await runAsync(`DELETE FROM order_operations WHERE order_id=?`, [id]);
    if (linkedExpenseRows.length) await runAsync(`DELETE FROM expenses WHERE id IN (${linkedExpenseRows.map(()=>'?').join(',')})`, linkedExpenseRows.map(r=>num(r.id)));
    await runAsync(`DELETE FROM orders WHERE id=?`, [id]);
    await runAsync(`DELETE FROM audit_logs WHERE (entity_type='order' AND entity_id=?) OR touch_refs LIKE ?`, [id, `%order:${id}%`]);
    return true;
  }

  if (type === 'order_operation' && id && (action === 'create-order-operation' || action === 'save-order-operation' || isCreateAction)) {
    const row = await getAsync(`SELECT * FROM order_operations WHERE id=?`, [id]);
  ...
  if (type === 'expense' && id && (isCreateAction || isDeleteAction)) {
    await deletePartnerFundEntriesBySource('expense', String(id));
    await deleteAdminCashEntriesBySource('expense', String(id));
    await runAsync(`DELETE FROM cost_history WHERE source='accounts' AND source_ref=?`, [String(id)]);
    await runAsync(`DELETE FROM expenses WHERE id=?`, [id]);
  ...
  if (type === 'cash_adjustment' && id && (isCreateAction || isDeleteAction || action === 'cash-adjustment')) {
    await runAsync(`DELETE FROM cash_adjustments WHERE id=?`, [id]);
    await runAsync(`DELETE FROM audit_logs WHERE entity_type='cash_adjustment' AND entity_id=?`, [id]);
    return true;
  }

  if (type === 'admin_cash' && id && (isCreateAction || isDeleteAction || action === 'transfer-admin-cash')) {
    await runAsync(`DELETE FROM admin_cash_ledger WHERE id=?`, [id]);
    await runAsync(`DELETE FROM audit_logs WHERE entity_type='admin_cash' AND entity_id=?`, [id]);
    return true;
  }

  if (type === 'partner' && id && (action === 'create-partner' || action === 'save-partner' || action === 'delete-partner')) {
  ...
  if (type === 'partner_payment' && id && (isCreateAction || isDeleteAction)) {
  ...
  if (type === 'purchase' && id && (action === 'save-purchase' || action === 'create-purchase' || action === 'delete-purchase')) {
    const purchase = await getAsync(`SELECT * FROM purchases WHERE id=?`, [id]);
    if (purchase) {
      const linkedPayments = await allAsync(`SELECT id FROM purchase_payments WHERE purchase_id=?`, [id]);
      if (num(purchase.stock_applied) === 1) await reversePurchaseStockSilently(purchase);
  ...
  if (type === 'purchase-payment' && id && (isCreateAction || isDeleteAction)) {
...
app.delete('/activity-logs/:id', authRequired, requirePerm('perm_activity_logs'), async (req, res) => {
  try {
    ...
    const dependentLogs = await collectDependentAuditLogs(log);
    await runAsync('BEGIN TRANSACTION');
    try {
      // نرجّع العمليات الأحدث المرتبطة أولاً، ثم العملية المطلوبة، عشان الداتا ترجع لحالتها قبل العملية المختارة.
      for (const row of [...dependentLogs].sort((a,b)=>num(b.id)-num(a.id))) {
        await deleteAuditLogTargetCascade(row, req);
      }
      try { await cleanupUndueOrderCostExpenses({ reason: 'admin-log-delete' }); } catch (_) {}
    ...
    res.json({ success: true, deleted_logs: dependentLogs.length, cascaded: dependentLogs.length > 1 ? 1 : 0 });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
async function runPostListenStartupMaintenance() {
  await runStartupTask('normalizeImportedBackupData', () => normalizeImportedBackupData({ reason: 'startup-schema-sync' }));
  await runStartupTask('resetExecutionOperationsOpenOnce', () => resetExecutionOperationsOpenOnce({ reason: 'startup-artisans-cleanup' }));
  await runStartupTask('syncOpeningCashFlowPaymentRows', () => syncOpeningCashFlowPaymentRows());
  await runStartupTask('syncAllSalesHistory', () => syncAllSalesHistory());
  await runStartupTask('cleanupLegacyAutoOrderCostCashExpenses', () => cleanupLegacyAutoOrderCostCashExpenses({ reason: 'startup-payment-model-migration' }));
  await runStartupTask('syncAllExecutionCostExpenses', () => syncAllExecutionCostExpenses({ reason: 'startup-execution-cost-sync' }));
  await runStartupTask('syncAllDueOrderCostExpenses', () => syncAllDueOrderCostExpenses({ reason: 'startup-stage-cost-sync' }));
  await runStartupTask('cleanupOrphanAutoFinancialLedgers', () => cleanupOrphanAutoFinancialLedgers());
  await runStartupTask('runAutomaticBackupIfNeeded', () => runAutomaticBackupIfNeeded());
...
ensureSchema().then(async () => {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    runPostListenStartupMaintenance().catch(err => console.error('startup maintenance failed', err?.message || err));
  });
