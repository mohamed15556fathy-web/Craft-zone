(function(){
    ...
    return `
      <div class="asset-form">
        ...
        <div class="form-grid">
          ...
          <div style="grid-column:1/-1" class="hint">شراء الأصل يُخصم تلقائيًا من الخزنة ومن صافي الربح في تاريخ الشراء، وتظل قيمته ظاهرة ضمن موجودات الشركة. مسموح أن تصبح الخزنة بالسالب أو يزيد الرصيد السالب.</div>
  ...
  async function renderFixedAssetsDetails(){
    ...
    byId('detailsNote').textContent = 'شراء الأصل يخصم من الخزنة وصافي الربح ويظل ضمن الموجودات';
    ...
    try {
      ...
      byId('detailsContent').innerHTML = `
        <div class="asset-summary-grid">
          <div class="asset-summary-item"><small>إجمالي قيمة الأصول</small><b>${moneyLocal(data.total_value)}</b></div>
  ...
  function renderProfitDetails(){
    ...
    byId('detailsNote').textContent = 'شراء الأصول مخصوم بالكامل من صافي الربح في فترة الشراء';
    const actualBeforeAssets = Number(accountsSummary.actualProfitBeforeAssets ?? 0);
    const assetPurchases = Number(accountsSummary.fixedAssetPurchases || 0);
    ...
    byId('detailsContent').innerHTML = detailsTable(['البند','القيمة'],[
      ...
      ['صافي الربح قبل شراء الأصول',moneyLocal(actualBeforeAssets)],
      ['شراء الأصول',`<span style="color:#fca5a5">- ${moneyLocal(assetPurchases)}</span>`],
      ['صافي الربح الفعلي بعد الأصول',`<b>${moneyLocal(actualProfit)}</b>`],
      ...
      ['صافي الربح بعد المتوقع وشراء الأصول',`<b>${moneyLocal(totalWithExpected)}</b>`]
    ]);
  }
  ...
  window.saveFixedAsset = async function(){
    ...
    try {
      ...
      alert(wasEditing ? 'تم تعديل الأصل بنجاح' : 'تم تسجيل شراء الأصل وخصمه من الخزنة وصافي الربح');
  ...
  window.deleteFixedAsset = async function(id){
    ...
    if (!confirm(`حذف الأصل ${row?.asset_name || ''}؟ سيتم عكس خصمه من الخزنة وصافي الربح.`)) return;
    try {
  ...
  function patchLoadCash(){
    ...
    window.loadCash = async function(){
      ...
      if (breakdown && canView && Number(cashSummary?.fixedAssetPurchases || 0) > 0) {
        breakdown.insertAdjacentHTML('beforeend', ` | شراء أصول: <b>${moneyLocal(cashSummary.fixedAssetPurchases)}</b>`);
      }
      return result;
    };
  }
