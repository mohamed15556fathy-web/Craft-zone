...
const vm = require('vm');
const htmlPath = path.join(__dirname, 'inventory.html');
const jsPath = path.join(__dirname, 'inventory.js');
...
const document = {
  ...
  addEventListener(){},
  createElement(tag){ const el = makeEl(tag); el.classList._owner = el; return el; },
  body: makeEl('body')
};
