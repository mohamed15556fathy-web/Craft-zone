...
function stepLabel(step){ return ({plate:'زنكات',print:'طباعة',lamination:'سلوفان',breaking:'تكسير',make:'تصنيع',handle:'تركيب يد'})[String(step||'').trim()] || step || '-'; }
...
function estimateOperationAmount(order, step){
  ...
  if(printType==='أوفست'){
    ...
    const usesForm=Number(order?.has_breaking||0)===1&&(Number(order?.form_length)||0)>0&&(Number(order?.form_width)||0)>0;
    const cutWidth = +(usesForm ? Number(order.form_width) : (mode==='pieceByPiece' ? ((Number(order?.w)||0) + (Number(order?.g)||0) + 2) : (((Number(order?.w)||0)*2) + ((Number(order?.g)||0)*2) + 2))).toFixed(2);
    const cutLength = +(usesForm ? Number(order.form_length) : ((Number(order?.l)||0) + ((Number(order?.g)||0)/2) + 2)).toFixed(2);
    ...
    const piecesPerBag=usesForm ? ((order?.form_construction_mode==='pieceByPiece'?2:1)/Math.max(1,Number(order?.form_piece_count)||1)) : (mode==='pieceByPiece'?2:1);
...
function stepPartnerType(step){ return ({plate:'زنكات',print:'مطبعة',lamination:'سلوفان',breaking:'تكسير',make:'صنايعي',handle:'تركيب يد'})[String(step||'').trim()] || ''; }
