...
function operationPartnerOptions(step, selected=''){
  ...
  return '<option value="">اختَر الجهة</option>' + rows.map(p=>`<option value="${p.id}" ${String(selected)===String(p.id)?'selected':''}>${esc(p.name)} - ${esc(p.partner_type||'')}</option>`).join('');
}
...
async function autoSaveOperationPartner(id, selectEl){
  ...
  if(!partnerId) return;
  const chosen=partners.find(p=>String(p.id)===String(partnerId));
  const prev={partner_id:row.partner_id, partner_name:row.partner_name, partner_type:row.partner_type};
  const body={id:Number(row.id||0),order_id:Number(row.order_id||0),partner_id:partnerId,step_type:String(row.step_type||''),status:String(row.status||'pending').trim()==='done'?'done':'pending',qty:Number(row.qty||0),amount:Number(row.amount||0),reference_code:String(row.reference_code||''),note:String(row.note||'')};
  ...
  try{
    ...
    updateOperationEverywhere(id,{partner_id:partnerId,partner_name:chosen?.name||row.partner_name||'',partner_type:chosen?.partner_type||row.partner_type||''});
    refreshOperationRowUi(id);
    showTinyStatus(id,'تم الحفظ',true);
  }catch(e){
